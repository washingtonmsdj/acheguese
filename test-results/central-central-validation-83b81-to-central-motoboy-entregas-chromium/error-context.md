...
- Name: central\central-validation.spec.ts >> Central - Redirects Legados >> should redirect /perfil/mobilidade/motoboy/entregas to /central/motoboy/entregas
- Location: tests\e2e\central\central-validation.spec.ts:100:3
...
Expected substring: "/central/motoboy/entregas"
Received string:    "http://localhost:8080/perfil/mobilidade/motoboy/entregas"
```
...
> 107 |     expect(page.url()).toContain('/central/motoboy/entregas');
  ...
  166 |   });
  167 | 
  168 |   test('should load /central/motorista/disponibilidade', async ({ page }) => {
  169 |     const response = await page.goto('/central/motorista/disponibilidade', {
  170 |       waitUntil: 'domcontentloaded',
  171 |       timeout: 30000,
  172 |     });
  173 | 
  174 |     expect(response?.status()).toBeLessThan(500);
  175 |   });
  176 | 
  177 |   test('should load /central/motorista/corridas', async ({ page }) => {
  178 |     const response = await page.goto('/central/motorista/corridas', {
  179 |       waitUntil: 'domcontentloaded',
  180 |       timeout: 30000,
  181 |     });
  182 | 
  183 |     expect(response?.status()).toBeLessThan(500);
  184 |   });
  185 | 
  186 |   test('should load /central/motorista/ganhos', async ({ page }) => {
  187 |     const response = await page.goto('/central/motorista/ganhos', {
  188 |       waitUntil: 'domcontentloaded',
  189 |       timeout: 30000,
  190 |     });
  191 | 
  192 |     expect(response?.status()).toBeLessThan(500);
  193 |   });
  194 | 
  195 |   test('should load /central/motorista/configuracoes', async ({ page }) => {
  196 |     const response = await page.goto('/central/motorista/configuracoes', {
  197 |       waitUntil: 'domcontentloaded',
  198 |       timeout: 30000,
  199 |     });
  200 | 
  201 |     expect(response?.status()).toBeLessThan(500);
  202 |   });
  203 | 
  204 |   test('should load /central/motoboy/cadastro', async ({ page }) => {
  205 |     const response = await page.goto('/central/motoboy/cadastro', {
