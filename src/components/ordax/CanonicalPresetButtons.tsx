...
export function CanonicalPresetButtons({ onPresetSelected }: CanonicalPresetButtonsProps) {
  ...
  const handlePresetClick = async (
    ...
    try {
      // 1. Gerar preset canônico
      const preset = presetFn();
      
      // 2. Converter para OrdaxSpec simplificado apenas para gerar resumo humano
      const ordaxSpec: Record<string, unknown> = {
        gameType: 'topdown',
        title: preset.name,
      ...
      // 3. Gerar resumos humanos
      const humanSummary = generateHumanSummary(ordaxSpec);
      const autofillReport: AutofillReport = {
        wasModified: false,
        summary: 'Preset canônico carregado. Autofill completo já embutido no preset.',
        added: {
          systems: [],
          entities: [],
          components: [],
      ...
      // 5. Passar para o componente pai
      onPresetSelected(preset, humanSummary, autofillReport);
