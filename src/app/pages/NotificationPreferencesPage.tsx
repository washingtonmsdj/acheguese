...
export default function NotificationPreferencesPage() {
  ...
  const quietHoursSummary = quietHoursPairInvalid
  ...
  const renderQuietHoursControls = (idPrefix: string) => {
    const startId = `${idPrefix}-quiet-start`;
    const endId = `${idPrefix}-quiet-end`;
    const errorId = `${idPrefix}-quiet-hours-error`;

    return (
      <>
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <Label htmlFor={startId} className="text-xs font-semibold text-territory-muted">Início</Label>
            <input
              id={startId}
              type="time"
              aria-invalid={quietHoursPairInvalid}
              aria-describedby={quietHoursPairInvalid ? errorId : undefined}
          ...
          <div>
            <Label htmlFor={endId} className="text-xs font-semibold text-territory-muted">Fim</Label>
            <input
              id={endId}
              type="time"
              aria-invalid={quietHoursPairInvalid}
    ...
    <>
      ...
      >
        ...
        <div className="mt-4 lg:hidden">
          <h2 className="mb-2 font-heading text-sm font-bold text-territory-ink">Horário de silêncio</h2>
          <details
            className="group rounded-2xl border border-territory-border bg-territory-surface"
            open={quietHoursInvalid || undefined}
          >
            <summary className="flex min-h-12 cursor-pointer list-none items-center gap-3 px-4 py-3 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-territory-brand [&::-webkit-details-marker]:hidden">
              <Clock3 className="h-5 w-5 shrink-0 text-territory-ink" aria-hidden="true" />
              <span className={cn("min-w-0 flex-1 text-sm font-medium", quietHoursInvalid ? "text-destructive" : "text-territory-ink")}>{quietHoursSummary}</span>
            ...
            <div className="border-t border-territory-border px-4 pb-4 pt-3">
              {renderQuietHoursControls("mobile")}
            </div>
          </details>
        ...
        <Surface className="mt-4 hidden p-4 sm:p-5 lg:block">
          ...
          <div className="mt-4">{renderQuietHoursControls("desktop")}</div>
        </Surface>
