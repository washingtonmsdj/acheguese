...
describe("BuscaPage", () => {
  beforeEach(() => {
    ...
    mocks.useGlobalSearch.mockImplementation((initialQuery, initialFilters, options) => ({
      query: initialQuery,
  ...
  it("hydrates URL query, applies territory and hides paused launch filters", () => {
    ...
    expect(screen.getByLabelText("Campo de busca")).toHaveValue("pizzaria");
    expect(screen.queryByRole("button", { name: /Eventos/i })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /Oportunidades/i })).not.toBeInTheDocument();
  ...
  it("renders federated non-business results in domain sections", () => {
    ...
    expect(screen.getByRole("heading", { name: "Classificados" })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Atividades nas comunidades" })).toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "Outros resultados" })).not.toBeInTheDocument();
  });
});
