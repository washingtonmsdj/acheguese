...
function findComplexTerritory(locations: Location[]): Location | null {
  ...
  return (
    locations.find(
      (location) =>
        normalizeTerritoryText(location.name).includes(normalizedName),
    ) ??
    locations.find((location) =>
      normalizeTerritoryText(location.name).includes("nordeste de amaralina"),
    ) ??
    (() => {
...
export default function TerritoryEntryPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    getLaunchTerritories().then((locations) => {
      if (!cancelled) setPreviewTerritory(findComplexTerritory(locations));
    });
    return () => {
      cancelled = true;
    };
        ...
        <section className="entry-left" aria-labelledby="territory-entry-title">
          <div className="entry-hero">
            ...
            <p className="entry-hero-subtitle">
              Negócios, serviços e histórias do Complexo do Nordeste de
          ...
          <section className="entry-selection" aria-labelledby="entry-community-title">
            <Link
              ...
              className="entry-community-preview"
              onClick={rememberComplex}
            >
              <img src={communityThumbnail} alt="" />
              <span>
                <strong id="entry-community-title">
                  {COMPLEX_TERRITORY_NAME}
                </strong>
