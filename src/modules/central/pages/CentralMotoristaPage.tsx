import { Car, MapPin, Route, Wallet, Settings2, FileText } from "lucide-react";
 ...
 * 
 * Página hub para motorista na Central (/central/motorista).
 * Mostra resumo operacional: status do perfil, disponibilidade, atalhos para corridas/ganhos/configurações.
...
export default function CentralMotoristaPage() {
  const driverIdentity = useDriverProfileIdentity({ queryScope: 'central-motorista' });
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center px-4">
        <div className="space-y-3 text-center">
          ...
          <p className="text-sm text-muted-foreground">Carregando perfil de motorista...</p>
        </div>
      </div>
    );
  }
        ...
        <Card className="max-w-md w-full">
          <CardContent className="space-y-4 p-6 text-center">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-semibold">Perfil de motorista não encontrado</h3>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Gerencie seu perfil de motorista e acompanhe suas operações
          </p>
        </div>
        <Badge variant={canDoRides ? "default" : "secondary"}>
          {canDoRides ? "Ativo" : "Inativo"}
        </Badge>
      </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {canDoRides && canDoDelivery ? 'Corridas e Entregas' : canDoRides ? 'Apenas Corridas' : 'Nenhuma'}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          <Button
            ...
            onClick={() => navigate(appUrls.profile.mobilidade.motorista.cadastro)}
          >
          ...
          <Button
            variant="outline"
            className="h-auto flex-col gap-2 py-6"
            onClick={() => navigate(appUrls.profile.mobilidade.motorista.disponibilidade)}
          >
          ...
          <Button
            variant="outline"
            className="h-auto flex-col gap-2 py-6"
            onClick={() => navigate(appUrls.profile.mobilidade.motorista.ganhos)}
          >
