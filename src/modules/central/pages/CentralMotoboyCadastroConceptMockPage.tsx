...
} from "@/core/mobility/utils/driverRegistration";
...
function DesktopSidebar({ step, view, status, onStep, onArea, onStatus }: { step: RegistrationStep; view: RegistrationView; status: RegistrationStatus; onStep: (step: RegistrationStep) => void; onArea: () => void; onStatus: (status: RegistrationStatus) => void }) {
  const navItem = (active = false) => cn("flex w-full items-center gap-3 rounded-lg px-2 py-2.5 text-left text-xs", active ? "bg-amber-50 font-bold text-territory-ink" : "text-territory-muted");
  const sidebarTop = view === "status" ? status === "correction" ? "pt-1.5" : "pt-[4.25rem]" : "pt-6";
  return <aside className="hidden min-h-0 border-r border-territory-border pr-5 md:block lg:pr-7"><div className={cn("sticky top-0", sidebarTop)}>{view === "form" ? <div className="space-y-4">{STEP_ITEMS.map(({ step: itemStep, label, icon: Icon }) => <button type="button" key={itemStep} onClick={() => onStep(itemStep)} className="flex w-full items-center gap-3 text-left"><span className={cn("flex h-7 w-7 items-center justify-center rounded-full border text-xs font-bold", step > itemStep ? "border-emerald-600 bg-emerald-600 text-white" : step === itemStep ? "border-territory-sun bg-territory-sun text-territory-ink" : "border-territory-border bg-territory-surface text-territory-muted")}>{step > itemStep ? <Check className="h-4 w-4" /> : itemStep}</span><span className={cn("text-xs", step === itemStep ? "font-bold text-territory-ink" : "text-territory-muted")}><Icon className="mr-1 inline h-3.5 w-3.5" />{label}</span></button>)}</div> : null}{view === "area" ? <div className="space-y-1"><button type="button" onClick={() => onStep(4)} className={navItem()}><FileCheck2 className="h-4 w-4" />Cadastro</button><button type="button" onClick={onArea} className={navItem(true)}><MapPin className="h-4 w-4" />Área de atuação</button><button type="button" onClick={() => toast.info("A ajuda ficará disponível quando conectada.")} className={navItem()}><CircleHelp className="h-4 w-4" />Ajuda</button></div> : null}{view === "status" ? <div className="space-y-1"><button type="button" onClick={() => onStep(4)} className={navItem(true)}><FileCheck2 className="h-4 w-4" />Cadastro</button><button type="button" onClick={() => onStatus("correction")} className={navItem()}><FileText className="h-4 w-4" />Documentos</button><button type="button" onClick={() => onStatus(status)} className={navItem()}><History className="h-4 w-4" />Histórico</button><button type="button" onClick={() => toast.info("A ajuda ficará disponível quando conectada.")} className={navItem()}><CircleHelp className="h-4 w-4" />Ajuda</button></div> : null}</div></aside>;
}

function MobileProgress({ step }: { step: RegistrationStep }) {
  const [params] = useSearchParams();
  if (params.get("status") || params.get("view") === "area") return null;
  return <div className="px-1 pt-1"><div className="flex items-center justify-center gap-1.5">{STEP_ITEMS.map(({ step: itemStep }) => <span key={itemStep} className={cn("h-2 w-2 rounded-full", itemStep === step ? "bg-territory-sun" : itemStep < step ? "bg-territory-brand" : "bg-slate-200")} />)}</div><p className="mt-1 text-center text-xs text-territory-muted">{step} de 4 · {STEP_ITEMS[step - 1].shortLabel}</p></div>;
}
...
export default function CentralMotoboyCadastroConceptMockPage() {
  ...
  return <div className="flex h-dvh min-h-0 flex-col overflow-hidden bg-[#fbfcfb] text-territory-ink"><Helmet><title>Cadastro do entregador | achegue-se.</title></Helmet><Header onBack={() => navigate("/central/motoboy?concept-mock=1&state=overview")} /><main id="cadastro" className="mx-auto flex min-h-0 w-full max-w-[1440px] flex-1 flex-col overflow-hidden px-4 py-4 sm:px-6 md:px-8 md:py-5"><div className="mb-3 md:hidden"><MobileProgress step={step} /></div><div className="grid min-h-0 flex-1 gap-5 md:grid-cols-[11rem_minmax(0,1fr)] lg:grid-cols-[13rem_minmax(0,1fr)]"><DesktopSidebar step={step} view={view} status={status} onStep={goStep} onArea={goArea} onStatus={goStatus} /><section className="flex min-h-0 flex-1 flex-col">{view === "status" ? <div className="mb-4 md:hidden"><RegistrationIdentity name={driverName} /></div> : null}<div className={cn("grid min-h-0 flex-1 gap-4", view === "area" ? "md:grid-cols-1 lg:grid-cols-1" : "md:grid-cols-1 lg:grid-cols-[minmax(0,1fr)_17rem]")}><div className={cn("flex min-h-0 flex-col overflow-hidden rounded-xl border border-territory-border bg-territory-surface p-4 sm:p-5 lg:p-6", view === "status" && "lg:rounded-none lg:border-0 lg:bg-transparent lg:p-0")}>{view === "area" ? <AreaContent selectedAreas={selectedAreas} onToggle={toggleArea} onSave={() => { toast.success("Área de atuação salva na demonstração."); goStep(4); }} /> : view === "status" ? <StatusContent status={status} onForm={(nextStep = 4) => goStep(nextStep)} onArea={goArea} onRetry={retryAnalysis} /> : <FormContent step={step} form={form} update={updateField} avatarName={avatarName} onAvatarChange={setAvatarName} onStep={goStep} onArea={goArea} onSubmit={submitRegistration} />}</div><DesktopProfileCard name={driverName} step={step} onReview={() => goStep(4)} onArea={goArea} /></div></section></div></main><footer className="mx-auto hidden w-full max-w-[1440px] items-center justify-between px-8 pb-4 text-[0.68rem] text-territory-muted md:flex"><span>Conceito proposto · Dados demonstrativos · A análise depende das regras da operação.</span><span>achegue-se<span className="text-territory-sun">.</span></span></footer></div>;
}
