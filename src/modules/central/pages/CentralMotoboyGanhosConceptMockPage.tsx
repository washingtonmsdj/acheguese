...
function DesktopShell({ children, navigate }: { children: ReactNode; navigate: (to: string) => void }) {
  return (
    <div className="hidden h-dvh min-h-0 overflow-hidden bg-[#fbfcfb] text-territory-ink md:flex">
      ...
      {DAILY_VALUES.map(({ day, value }) => (
        <div key={day} className="flex h-full flex-1 flex-col items-center justify-end gap-1.5">
          ...
          <div className="flex w-full flex-1 items-end justify-center">
            <div className={cn("w-full max-w-8 rounded-t-md bg-territory-sun", mobile ? "min-h-4" : "min-h-5")} style={{ height: `${(value / 30) * 100}%` }} />
          </div>
...
function MobileDeliveryDetail({ onView }: { onView: (view: GainsView) => void }) {
  return <div className="flex h-full min-h-0 flex-col"><MobileHero view="delivery-detail" title="Entrega #1042" subtitle="Carlos Santos · Motoboy" /><div className="min-h-0 flex-1 overflow-hidden bg-[#fbfcfb] px-3 pb-[4.25rem] pt-3"><div className="flex h-full min-h-0 flex-col gap-2.5"><section className="rounded-xl border border-territory-border bg-white p-3"><div className="flex items-start justify-between"><div><p className="text-sm font-bold">Sabores da Ana</p><p className="text-xs text-territory-muted">13 set. 2026 · 12:46</p></div><span className="rounded-full bg-[#baf0d1] px-2.5 py-1 text-[0.68rem] font-bold text-[#08734e]">Entregue</span></div><div className="mt-3 border-t pt-3"><p className="text-xs text-territory-muted">Valor registrado</p><p className="font-heading text-3xl font-extrabold">R$ 7,00</p></div></section><section className="rounded-xl border border-territory-border bg-white p-3"><div className="flex gap-3"><MapPin className="h-5 w-5 shrink-0 text-emerald-600" /><div><p className="text-xs font-bold">Coleta</p><p className="text-xs text-territory-muted">Santa Cruz, Salvador – BA</p></div></div><div className="mt-3 flex gap-3 border-t pt-3"><MapPin className="h-5 w-5 shrink-0 text-amber-500" /><div><p className="text-xs font-bold">Destino</p><p className="text-xs text-territory-muted">Nordeste de Amaralina, Salvador – BA</p></div></div></section><section className="rounded-xl border border-territory-border bg-white p-3"><p className="text-xs font-bold">Recebimento</p><p className="mt-1 text-sm font-bold">Não confirmado</p><p className="mt-1 text-xs text-territory-muted">Não há confirmação financeira vinculada.</p></section><section className="rounded-xl border border-territory-border bg-white p-3"><div className="flex items-center justify-between"><div><p className="text-xs font-bold">Composição do valor</p><p className="mt-1 text-xs text-territory-muted">Detalhamento não disponível</p></div><FileText className="h-5 w-5 text-territory-muted" /></div><button type="button" onClick={() => toast.info("Detalhamento não disponível neste conceito.")} className="mt-2 text-xs font-semibold text-blue-700">Entender este valor</button></section><div className="mt-auto space-y-2"><button type="button" onClick={() => toast.info("Comprovante de entrega aberto.")} className="flex w-full items-center justify-between px-1 text-xs font-semibold text-blue-700"><span className="flex items-center gap-2"><FileText className="h-4 w-4" />Ver pedido e comprovante de entrega</span><ArrowRight className="h-4 w-4" /></button><button type="button" onClick={() => toast.info("Histórico aberto.")} className="flex w-full items-center justify-between px-1 text-xs font-semibold text-blue-700"><span className="flex items-center gap-2"><History className="h-4 w-4" />Histórico de alterações</span><ArrowRight className="h-4 w-4" /></button><ActionButton className="w-full" onClick={() => onView("review")}>Solicitar revisão</ActionButton></div></div></div></div>;
}
