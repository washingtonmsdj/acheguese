...
export class MobilityServiceRefactored {
  ...
  async getAllRideRequests(): Promise<Ride[]> {
    const orderBy = [this.rideRepository.createOrderBy('created_at', 'asc')];
  ...
  async assignDriver(rideId: string, driverId: string): Promise<Ride> {
    return await this.rideRepository.assignDriver(rideId, driverId);
  }
   ...
   */
  async countRidesByDriver(driverId: string): Promise<number> {
    try {
      return await this.rideRepository.countByDriverId(driverId);
    } catch (error) {
  ...
  async updateDriverAvailability(
    ...
    return await this.driverRepository.updateAvailability(driverId, isAvailable);
  }
   ...
   */
  async incrementDriverRides(driverId: string): Promise<Driver> {
    return await this.driverRepository.incrementTotalRides(driverId);
  }

  /**
   * Incrementa ganhos do motorista
   */
  async incrementDriverEarnings(driverId: string, amount: number): Promise<Driver> {
    return await this.driverRepository.incrementEarnings(driverId, amount);
  }
  ...
  async updateDriverRating(driverId: string, newRating: number): Promise<Driver> {
    return await this.driverRepository.updateRating(driverId, newRating);
  }
  ...
  async suspendDriver(driverId: string): Promise<Driver> {
    return await this.driverRepository.suspend(driverId);
  }
  ...
  async reactivateDriver(driverId: string): Promise<Driver> {
    return await this.driverRepository.reactivate(driverId);
  }
