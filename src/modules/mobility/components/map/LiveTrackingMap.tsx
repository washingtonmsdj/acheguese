...
export function LiveTrackingMap({
  ...
  const canTrackDriver = isDriverOwnedOpenRideStatus(rideStatus);
  const canonicalStatus = toCanonicalRideState(rideStatus);
  const { location, loading } = useDriverLocation({
    driverProfileId,
  ...
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !location || !canTrackDriver) return;
    ...
    if (canonicalStatus === RIDE_STATE.IN_PROGRESS) {
  ...
  }, [location, canonicalStatus, canTrackDriver]);

  const isActive = canonicalStatus === RIDE_STATE.IN_PROGRESS;
  const isOnWay = canTrackDriver && !isActive;
  const statusText = isActive
    ? 'Viagem em andamento'
    : isOnWay
      ? 'Motorista confirmado'
      : isPreAcceptRideStatus(rideStatus)
