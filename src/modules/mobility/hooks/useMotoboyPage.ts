 ...
 *
 * O dashboard-base tem "corridas" como aba inicial porque seu caller primario e
 * o painel de motorista. O owner de motoboy normaliza apenas esse default para
 * a primeira aba valida do dominio, sem criar um segundo estado de tabs.
 */
export function useMotoboyPage() {
  ...
  const activeTab = baseHook.activeTab === "corridas" ? "entregas" : baseHook.activeTab;
  ...
  const handleGoToPickup = useCallback(
  ...
  const handleConfirmPickup = useCallback(
    async (rideId: string, driverProfileId: string) => {
      ...
      setDeliveryActionsLoading(true);
      try {
        const result = await RideOperationalService.confirmPickup(
          rideId,
          driverProfileId,
        );
        if (result.success) {
  ...
  const handleStartDelivery = useCallback(
    async (rideId: string, driverProfileId: string) => {
      ...
      setDeliveryActionsLoading(true);
      try {
        const result = await RideOperationalService.startDelivery(
          rideId,
          driverProfileId,
        );
        if (result.success) {
