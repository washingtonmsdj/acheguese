...
export class OrderDraftService {
  static normalizeSourceContext(
    ...
    if (normalizedSourceType !== ORDER_SOURCE_TYPE.MANUAL && !normalizedSourceId) {
      throw new Error(
        "source_id é obrigatório quando source_type não é manual.",
      );
    }
  ...
  static normalizeItems(items?: CreateOrderItemInput[]): CreateOrderItemInput[] {
    ...
    return items.map((item, index) => {
      ...
      if (!name) {
        throw new Error(`Item ${index + 1}: name é obrigatório.`);
      }
      ...
      if (unitPrice < 0 || addonsTotal < 0 || lineTotal < 0) {
        throw new Error(
          `Item ${index + 1}: unit_price, addons_total e line_total não podem ser negativos.`,
        );
      }

      if (lineTotal !== computedLineTotal) {
        throw new Error(
          `Item ${index + 1}: line_total divergente do cálculo esperado.`,
        );
      }
  ...
  static assertItemsTotalMatchesFinancial(
    ...
    if (normalizedItemsTotal !== calculatedItemsTotal) {
      throw new Error(
        `items_total divergente do somatório dos itens (${calculatedItemsTotal.toFixed(2)}).`,
      );
    }
  }
}
