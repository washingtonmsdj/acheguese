...
- O bloco `CENTRAL` duplicado e pausado em `src/app/routes/lazyImports.ts` foi removido; a árvore real `/central/*` usa `activeCentralLazyImports.ts`; Gastronomia permanece preservada no bounded context e fora do grafo privado ativo enquanto `gastronomy` estiver pausado.
...
- O dashboard operacional, o widget de plano e o card de acoes rapidas deixaram de exibir CTAs/beneficios para Entregas avancadas, Analytics e Promocoes enquanto Gastronomia permanece fora de `CentralRoutes`/`activeCentralLazyImports.ts`.
