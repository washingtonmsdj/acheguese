...
import type { UseOrderTrackingResult } from "../hooks/useOrderTracking";
...
export default function OrderTrackingConceptSurface({
  ...
  const estimatedMinutes = rideRequest?.estimated_duration ?? order.estimated_delivery_time;
  ...
  const heroDescription =
    ...
    sourceMetadata.delivery_estimate_note.trim()
      ...
      : state === "stale" && rideRequest?.updated_at
        ? `Última posição recebida às ${formatTime(rideRequest.updated_at) ?? "não informada"}.`
      : hero.description;
  const staleNote =
    state === "stale"
      ...
      : null;
  const driverName = rideRequest?.driver?.name ?? rideRequest?.driver_profile?.name ?? null;
  const driverPhone = rideRequest?.driver?.phone ?? null;
  const driverAvatarUrl = rideRequest?.driver?.profile?.avatar_url ?? null;
          ...
          >
            <div className={cn(
              ...
              <div className="min-w-0">
                ...
                {state === "stale" && rideRequest?.updated_at && !staleNote ? (
                  <p className="mt-1 text-xs font-semibold text-territory-ink">
                    Última atualização: {formatTime(rideRequest.updated_at) ?? "não informada"}
                  </p>
                ) : null}
