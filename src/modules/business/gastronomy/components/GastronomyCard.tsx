      ...
      >
        ...
        <div className="relative aspect-[16/10] overflow-hidden">
          {business.banner_url ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-orange-500/10 to-red-500/10">
              <Store className="h-16 w-16 text-orange-500/50" />
            </div>
          )}
          ...
          {onToggleFavorite && (
            <button
              onClick={handleFavoriteClick}
              className="absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-full bg-black/50 backdrop-blur-sm transition-transform hover:scale-110"
              aria-label={isFavorite ? 'Remover dos favoritos' : 'Adicionar aos favoritos'}
            >
              <Heart
                className={cn(
              ...
              <div className="flex items-center gap-3 text-xs text-white">
                ...
                {gp.delivery_fee !== undefined && gp.delivery_fee !== null && (
                  <span className="flex items-center gap-1 font-medium">
                    ...
                    {gp.delivery_fee === 0 ? 'Grátis' : formatBrl(gp.delivery_fee)}
                  </span>
                )}
                {gp.minimum_order && (
                  <span className="flex items-center gap-1 text-[10px] opacity-80">
                    <Package className="h-3 w-3" />
                    Mín. {formatBrl(gp.minimum_order)}
                  </span>
                )}
              </div>
            </div>
        ...
        <div className="flex flex-1 flex-col gap-3 p-4">
          ...
          <div className="min-h-[3.5rem]">
            ...
            <p className="mt-0.5 text-sm text-muted-foreground">{cuisineLabel}</p>
          </div>
          ...
          {/* Badges Secundárias */}
          {(isFreeDelivery || showPromotion || gp.takeout_enabled || gp.dine_in_enabled) && (
          ...
          <Button
            variant="default"
            className="mt-auto w-full font-semibold transition-all group-hover:shadow-lg"
            size="sm"
          >
            Ver cardápio
            <motion.span
