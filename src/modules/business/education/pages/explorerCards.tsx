  ...
  if (view === 'list') {
    return (
      <motion.article initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * 0.03, 0.3) }} className="group relative grid grid-cols-1 gap-0 overflow-hidden rounded-2xl border border-border bg-card transition-all hover:border-primary/30 hover:shadow-lg md:grid-cols-[220px_1fr_220px]">
        <div className={cn('relative flex h-32 items-center justify-center bg-gradient-to-br md:h-full', gradient)}><Icon className="h-12 w-12 text-white drop-shadow" /></div>
        ...
        <div className="flex flex-col justify-between gap-2 border-l border-border/60 p-5">
          <button onClick={() => onCompareToggle(profile.id)} className={cn('inline-flex items-center justify-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium transition-colors', comparing ? 'border-primary bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:text-foreground')}><ScanSearch className="h-3.5 w-3.5" />{comparing ? 'Adicionado' : 'Comparar'}</button>
          ...
          {whatsappHref && <Button size="sm" variant="outline" className="w-full justify-between rounded-full" asChild><SafeLink href={whatsappHref} target="_blank">WhatsApp <MessageCircle className="h-4 w-4" /></SafeLink></Button>}
        </div>
      </motion.article>
    );
  }
      ...
      <div className="flex flex-1 flex-col p-3">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 space-y-2">
            ...
            <div className="flex min-w-0 flex-wrap items-center gap-1.5"><Badge variant="secondary" className="px-2 py-0 text-[10px]">{profile.school_network ? schoolNetworkLabels[profile.school_network] ?? 'Escola' : nicheConfig?.displayName ?? profile.niche_key}</Badge></div>
          </div>
        ...
        <div className="mt-3 flex gap-1.5 border-t border-border/60 pt-3">
          ...
          {whatsappHref && <Button size="sm" variant="outline" className="h-8 rounded-full px-2.5" asChild><SafeLink href={whatsappHref} target="_blank"><MessageCircle className="h-3.5 w-3.5" /></SafeLink></Button>}
        </div>
      </div>
    </motion.article>
  );
