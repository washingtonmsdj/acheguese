 ...
 * GroupsView
 * 
 * Visualização focada em grupos territoriais.
 * Exibe cards de grupos com seus bairros membros em grid compacto.
 * 
 * SSOT: Props tipadas vindas de sections/types.ts
 * Sem gambiarras: Componente focado apenas em renderização
 */

import { Users, MapPin, Eye, Edit } from 'lucide-react';
import { cn } from '@/shared/utils/cn';
import { Badge } from '@/shared/components/ui/badge';
import { Button } from '@/shared/components/ui/button';
import { Switch } from '@/shared/components/ui/switch';
import type { GroupsViewProps } from '../../sections/types';
  ...
  return (
    <div className="space-y-3">
      {groups.map(group => {
        ...
        const memberLocations = allLocations.filter(loc => memberIds.includes(loc.id));
        const anchorCity = allLocations.find(loc => loc.id === group.parent_id);
