...
export class AdminVagasService {
  static async getStats(): Promise<VagaStats> {
    try {
      ...
      logger.error("[AdminVagasService] Erro ao buscar estat�sticas", error);
      throw error;
    }
  }
  ...
  static async toggleUrgencia(vagaId: string, urgente: boolean): Promise<boolean> {
    try {
      ...
      logger.error("[AdminVagasService] Erro ao alterar urg�ncia", error);
      return false;
    }
  }
  ...
  private static mapRowToAdminVaga(row: any): AdminVaga {
    ...
    return {
      id: row.id,
      titulo: row.titulo,
      empresa: row.empresa_nome ?? row.empresa ?? "Empresa n�o informada",
