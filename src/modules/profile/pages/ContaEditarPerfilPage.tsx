      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <Field id="company_type" label="Tipo">
          <Select
            ...
            onValueChange={(v) =>
              set("company_type", v as BusinessData["company_type"])
            }
          >
...
function ProfessionalSection({
  ...
  const set = <K extends keyof ProfessionalData>(
    key: K,
    ...
    <div className="space-y-4">
      ...
      <Field id="profession" label="Profissão *">
        <Input
          ...
          placeholder="Ex: Eletricista, Designer, Advogado"
        />
      </Field>
      ...
      <Field id="specialties" label="Especialidades" hint="Separe por vírgula">
        <Input
          id="specialties"
          value={arrToStr(data.specialties)}
      ...
      >
        <Input
          id="services_offered"
          value={arrToStr(data.services_offered)}
      ...
      <Field id="education" label="Formação">
        <Input
          ...
          placeholder="Ex: Técnico em Eletrotécnica - SENAI"
        />
      </Field>
      ...
      >
        <Input
          id="certifications"
          value={arrToStr(data.certifications)}
      ...
      >
        <Select
          ...
          onValueChange={(v) =>
            set(
              "visibility",
              v as "public_listed" | "public_unlisted" | "private",
            )
          }
        >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <Field id="license_category" label="Categoria *">
          <Select
            ...
            onValueChange={(v) => set("license_category", v)}
          >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Field id="vehicle_type" label="Tipo">
          <Select
            ...
            onValueChange={(v) =>
              set("vehicle_type", v as DriverData["vehicle_type"])
            }
          >
      ...
      <div className="mx-auto max-w-[1180px] px-3 pb-24 pt-3 sm:px-6 sm:pb-10 sm:pt-5 lg:px-8">
        ...
        <div className="grid gap-4 lg:grid-cols-[260px_minmax(0,1fr)] lg:gap-5">
          <aside className="hidden lg:block">
            <div className="sticky top-6 space-y-3 rounded-territory-highlight border border-territory-border bg-territory-raised p-4">
          ...
          <main className="space-y-4 sm:space-y-5">
            <section className="rounded-territory-highlight border border-territory-border bg-territory-surface p-4 sm:p-6">
              <div className="space-y-4">
                ...
                <Field id="display_name" label="Nome de exibição *">
                  <Input
                    id="display_name"
                    value={baseForm.display_name ?? ""}
                    onChange={(e) =>
                      setBaseField("display_name", e.target.value)
                    }
                  />
                </Field>
                ...
                <Field id="bio" label="Bio">
                  <Textarea
                    id="bio"
                    value={baseForm.bio ?? ""}
                    onChange={(e) => setBaseField("bio", e.target.value)}
                    rows={3}
                ...
                {profileType === "personal" && (
                  <Field
                    id="short_bio"
                    label="Bio curta social"
                    hint="Resumo curto para identidade social (máx. 280)"
                  >
                    <Textarea
                      id="short_bio"
                      value={baseForm.short_bio ?? ""}
                      onChange={(e) =>
                        setBaseField("short_bio", e.target.value.slice(0, 280))
                      }
                      rows={2}
                ...
                <Field id="website" label="Website">
                  <Input
                    id="website"
                    type="url"
                    value={baseForm.website ?? ""}
                    onChange={(e) => setBaseField("website", e.target.value)}
                    placeholder="https://"
                  />
                </Field>
                ...
                {profileType !== "personal" && (
                  <Field id="location_id" label="Bairro (canônico)">
                    <LocationFields
                      ...
                      onChange={(locationId) =>
                        setBaseField("location_id", locationId ?? undefined)
                      }
                    />
                  </Field>
                )}
                ...
                {profileType === "personal" && (
                  <div className="rounded-2xl border border-border bg-muted/30 p-3">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      A participação em grupos é vinculada automaticamente pela
                      sua atuação em comunidades e organizações.
                    </p>
                  </div>
                )}
                ...
                <div className="rounded-2xl border border-border bg-muted/30 p-3">
                  <p className="text-sm font-medium">
                    Endereço de entrega não é salvo aqui
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Esta tela edita apenas identidade pública, como bio e
                    visibilidade de localização. Endereço completo de entrega é
                    restrito e fica somente em Configurações operacionais.
                  </p>
                  <Button
                    type="button"
                    variant="outline"
                ...
                <Field
                  ...
                  hint="Diferente do e-mail de login"
                >
                  <Input
                    id="contact_email"
                    type="email"
                    ...
                    onChange={(e) =>
                      setBaseField("contact_email", e.target.value)
                    }
                  />
                </Field>
                ...
                <ToggleRow
                  ...
                  description="Aparece em buscas e na URL /u/:username"
                  checked={baseForm.is_public ?? false}
                  onChange={(v) => setBaseField("is_public", v)}
                />
                {profileType === "personal" && (
                  <Field
                  ...
                  >
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="city_only">
                          Mostrar somente cidade/UF
                        </SelectItem>
                        <SelectItem value="district">
                          Mostrar bairro + cidade/UF
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  </Field>
                ...
                <ToggleRow
                  label="Mostrar e-mail de contato"
                  checked={baseForm.show_contact_email ?? false}
                  onChange={(v) => setBaseField("show_contact_email", v)}
                />
                <ToggleRow
                  label="Mostrar telefone"
                  checked={baseForm.show_phone ?? false}
                  onChange={(v) => setBaseField("show_phone", v)}
                />
                <ToggleRow
                  label="Mostrar vínculos"
                  checked={baseForm.show_linked_profiles ?? false}
                  onChange={(v) => setBaseField("show_linked_profiles", v)}
                />
              </div>
            </section>
            ...
            <div className="sticky bottom-0 -mx-3 flex gap-3 border-t border-territory-border bg-territory-canvas/95 px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-3 backdrop-blur sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:px-0 sm:pb-8 sm:pt-0">
              <Button
                variant="outline"
