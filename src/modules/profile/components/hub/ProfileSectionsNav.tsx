...
function TabsVariant<TId extends string>({
  ...
  return (
    <nav
      aria-label="Secoes do perfil"
      className="sticky top-0 z-20 -mx-3 border-b border-border/60 bg-background/88 px-3 py-2 backdrop-blur-md sm:-mx-0 sm:rounded-2xl sm:border sm:bg-card/92"
    >
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                onClick={() => onChange(item.id)}
                className={cn(
                  "flex min-h-10 shrink-0 items-center gap-1.5 rounded-full border px-3 py-2 text-xs font-medium transition-all",
                  isActive
