...
const nodeEnv =
  typeof process !== "undefined" ? (process.env as PublicEnv) : {};
const viteEnv = ((import.meta as ImportMeta & { env?: PublicEnv }).env ??
  {}) as PublicEnv;
const publicEnv: PublicEnv = { ...nodeEnv, ...viteEnv };

const launchCountry =
  publicEnv.VITE_LAUNCH_COUNTRY?.trim().toLowerCase() || "br";
...
const launchName =
  publicEnv.VITE_LAUNCH_CITY_NAME?.trim() || "Território inicial";
...
export const LAUNCH_CITY_PATH =
  launchState && launchCity ? `/${launchState}/${launchCity}` : "/brasil";
...
export const LAUNCH_URLS = {
  ...
  classifieds: buildAppModulePath(
  ...
  touristPoints: buildAppModulePath(
