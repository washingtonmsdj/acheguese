/**
 * TerritorialSelector - Selecao hierarquica de territorio (Estado > Cidade > Bairro)
 *
 * Usa o SSOT territorial (tabela locations) via useLocationCascade.
 */
...
export function TerritorialSelector({
  ...
  const [resolvedLabels, setResolvedLabels] = useState<SelectedLocationData | null>(null);
  ...
  useEffect(() => {
    if (!initialLocationId || selectedStateId || isResolvingLocation) {
      return;
    }
    ...
    Promise.all([
      ...
      .then(([currentLocation, ancestors]) => {
        ...
        const district = ancestors.find((location) => location.type === "district");
  ...
  const selectedLocationData = useMemo<SelectedLocationData | null>(() => {
    ...
    const neighborhood = neighborhoods.find((item) => item.id === selectedNeighborhoodId);
  ...
  useEffect(() => {
    ...
    const selectedId = cityOnly || (allowCityOnly && !selectedNeighborhoodId)
      ? selectedLocationData.cityId
      : selectedLocationData.neighborhoodId;

    onLocationChange(selectedId, selectedLocationData);
  }, [allowCityOnly, cityOnly, isResolvingLocation, onLocationChange, selectedLocationData, selectedNeighborhoodId]);
          ...
          {loadingNeighborhoods ? (
            ...
            >
              <SelectTrigger id="territorial-neighborhood">
                <SelectValue placeholder={selectedCityId ? "Selecione o bairro" : "Selecione a cidade primeiro"} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="none" disabled>
                  Selecione o bairro
