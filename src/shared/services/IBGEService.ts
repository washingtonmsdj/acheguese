/**
 * IBGE Service - Integração com API do IBGE
 * 
 * Fornece dados oficiais de localização do Brasil:
 * - Estados (UF)
 * - Municípios (Cidades)
 * - Distritos e Subdistritos (Bairros)
 * 
 * @version 1.0.0
 */

import { logger } from "@/shared/utils/logger";

// ============================================================================
// TYPES
// ============================================================================

export interface Estado {
  id: number;
  sigla: string;
  nome: string;
  regiao: {
    id: number;
    sigla: string;
    nome: string;
  };
}

export interface Municipio {
  id: number;
  nome: string;
  microrregiao: {
    id: number;
    nome: string;
    mesorregiao: {
      id: number;
      nome: string;
      UF: {
        id: number;
        sigla: string;
        nome: string;
      };
    };
  };
}

export interface Distrito {
  id: number;
  nome: string;
  municipio: {
    id: number;
    nome: string;
  };
}

export interface LocalidadeCompleta {
  estado: string;
  estadoNome: string;
  cidade: string;
  cidadeId: number;
  bairro?: string;
}

// ============================================================================
// CACHE
// ============================================================================

class IBGECache {
  private estados: Estado[] | null = null;
  private municipiosPorEstado: Map<string, Municipio[]> = new Map();
  private distritosPorMunicipio: Map<number, Distrito[]> = new Map();
  private ttl = 24 * 60 * 60 * 1000; // 24 horas
  private lastUpdate = 0;

  isValid(): boolean {
    return Date.now() - this.lastUpdate < this.ttl;
  }

  setEstados(estados: Estado[]): void {
    this.estados = estados;
    this.lastUpdate = Date.now();
  }

  getEstados(): Estado[] | null {
    return this.isValid() ? this.estados : null;
  }

  setMunicipios(uf: string, municipios: Municipio[]): void {
    this.municipiosPorEstado.set(uf, municipios);
  }

  getMunicipios(uf: string): Municipio[] | null {
    return this.isValid() ? this.municipiosPorEstado.get(uf) || null : null;
  }

  setDistritos(municipioId: number, distritos: Distrito[]): void {
    this.distritosPorMunicipio.set(municipioId, distritos);
  }

  getDistritos(municipioId: number): Distrito[] | null {
    return this.isValid() ? this.distritosPorMunicipio.get(municipioId) || null : null;
  }

  clear(): void {
    this.estados = null;
    this.municipiosPorEstado.clear();
    this.distritosPorMunicipio.clear();
    this.lastUpdate = 0;
  }
}

const cache = new IBGECache();

// ============================================================================
// SERVICE
// ============================================================================

export class IBGEService {
  private static readonly BASE_URL = "https://servicodados.ibge.gov.br/api/v1/localidades";

  /**
   * Buscar todos os estados do Brasil
   */
  static async getEstados(): Promise<Estado[]> {
    try {
      // Verificar cache
      const cached = cache.getEstados();
      if (cached) {
        return cached;
      }

      const response = await fetch(`${this.BASE_URL}/estados?orderBy=nome`);
      
      if (!response.ok) {
        throw new Error(`IBGE API error: ${response.status}`);
      }

      const estados: Estado[] = await response.json();
      
      // Salvar no cache
      cache.setEstados(estados);
      
      logger.info("Estados carregados do IBGE", {
        action: "getEstados",
        count: estados.length,
      });

      return estados;
    } catch (error) {
      logger.error("Erro ao buscar estados do IBGE", error as Error, {
        action: "getEstados",
      });
      
      // Retornar lista básica em caso de erro
      return this.getEstadosBasicos();
    }
  }

  /**
   * Buscar municípios de um estado
   */
  static async getMunicipiosPorEstado(uf: string): Promise<Municipio[]> {
    try {
      // Verificar cache
      const cached = cache.getMunicipios(uf);
      if (cached) {
        return cached;
      }

      const response = await fetch(
        `${this.BASE_URL}/estados/${uf}/municipios?orderBy=nome`
      );
      
      if (!response.ok) {
        throw new Error(`IBGE API error: ${response.status}`);
      }

      const municipios: Municipio[] = await response.json();
      
      // Salvar no cache
      cache.setMunicipios(uf, municipios);
      
      logger.info("Municípios carregados do IBGE", {
        action: "getMunicipiosPorEstado",
        uf,
        count: municipios.length,
      });

      return municipios;
    } catch (error) {
      logger.error("Erro ao buscar municípios do IBGE", error as Error, {
        action: "getMunicipiosPorEstado",
        uf,
      });
      return [];
    }
  }

  /**
   * Buscar distritos de um município
   */
  static async getDistritosPorMunicipio(municipioId: number): Promise<Distrito[]> {
    try {
      // Verificar cache
      const cached = cache.getDistritos(municipioId);
      if (cached) {
        return cached;
      }

      const response = await fetch(
        `${this.BASE_URL}/municipios/${municipioId}/distritos?orderBy=nome`
      );
      
      if (!response.ok) {
        throw new Error(`IBGE API error: ${response.status}`);
      }

      const distritos: Distrito[] = await response.json();
      
      // Salvar no cache
      cache.setDistritos(municipioId, distritos);
      
      logger.info("Distritos carregados do IBGE", {
        action: "getDistritosPorMunicipio",
        municipioId,
        count: distritos.length,
      });

      return distritos;
    } catch (error) {
      logger.error("Erro ao buscar distritos do IBGE", error as Error, {
        action: "getDistritosPorMunicipio",
        municipioId,
      });
      return [];
    }
  }

  /**
   * Buscar município por nome e estado
   */
  static async buscarMunicipio(
    nome: string,
    uf: string
  ): Promise<Municipio | null> {
    try {
      const municipios = await this.getMunicipiosPorEstado(uf);
      return (
        municipios.find(
          (m) => m.nome.toLowerCase() === nome.toLowerCase()
        ) || null
      );
    } catch (error) {
      logger.error("Erro ao buscar município", error as Error, {
        action: "buscarMunicipio",
        nome,
        uf,
      });
      return null;
    }
  }

  /**
   * Limpar cache (útil para testes ou forçar atualização)
   */
  static clearCache(): void {
    cache.clear();
    logger.info("Cache do IBGE limpo", { action: "clearCache" });
  }

  /**
   * Lista básica de estados (fallback)
   */
  private static getEstadosBasicos(): Estado[] {
    return [
      { id: 12, sigla: "AC", nome: "Acre", regiao: { id: 1, sigla: "N", nome: "Norte" } },
      { id: 27, sigla: "AL", nome: "Alagoas", regiao: { id: 2, sigla: "NE", nome: "Nordeste" } },
      { id: 16, sigla: "AP", nome: "Amapá", regiao: { id: 1, sigla: "N", nome: "Norte" } },
      { id: 13, sigla: "AM", nome: "Amazonas", regiao: { id: 1, sigla: "N", nome: "Norte" } },
      { id: 29, sigla: "BA", nome: "Bahia", regiao: { id: 2, sigla: "NE", nome: "Nordeste" } },
      { id: 23, sigla: "CE", nome: "Ceará", regiao: { id: 2, sigla: "NE", nome: "Nordeste" } },
      { id: 53, sigla: "DF", nome: "Distrito Federal", regiao: { id: 5, sigla: "CO", nome: "Centro-Oeste" } },
      { id: 32, sigla: "ES", nome: "Espírito Santo", regiao: { id: 3, sigla: "SE", nome: "Sudeste" } },
      { id: 52, sigla: "GO", nome: "Goiás", regiao: { id: 5, sigla: "CO", nome: "Centro-Oeste" } },
      { id: 21, sigla: "MA", nome: "Maranhão", regiao: { id: 2, sigla: "NE", nome: "Nordeste" } },
      { id: 51, sigla: "MT", nome: "Mato Grosso", regiao: { id: 5, sigla: "CO", nome: "Centro-Oeste" } },
      { id: 50, sigla: "MS", nome: "Mato Grosso do Sul", regiao: { id: 5, sigla: "CO", nome: "Centro-Oeste" } },
      { id: 31, sigla: "MG", nome: "Minas Gerais", regiao: { id: 3, sigla: "SE", nome: "Sudeste" } },
      { id: 15, sigla: "PA", nome: "Pará", regiao: { id: 1, sigla: "N", nome: "Norte" } },
      { id: 25, sigla: "PB", nome: "Paraíba", regiao: { id: 2, sigla: "NE", nome: "Nordeste" } },
      { id: 41, sigla: "PR", nome: "Paraná", regiao: { id: 4, sigla: "S", nome: "Sul" } },
      { id: 26, sigla: "PE", nome: "Pernambuco", regiao: { id: 2, sigla: "NE", nome: "Nordeste" } },
      { id: 22, sigla: "PI", nome: "Piauí", regiao: { id: 2, sigla: "NE", nome: "Nordeste" } },
      { id: 33, sigla: "RJ", nome: "Rio de Janeiro", regiao: { id: 3, sigla: "SE", nome: "Sudeste" } },
      { id: 24, sigla: "RN", nome: "Rio Grande do Norte", regiao: { id: 2, sigla: "NE", nome: "Nordeste" } },
      { id: 43, sigla: "RS", nome: "Rio Grande do Sul", regiao: { id: 4, sigla: "S", nome: "Sul" } },
      { id: 11, sigla: "RO", nome: "Rondônia", regiao: { id: 1, sigla: "N", nome: "Norte" } },
      { id: 14, sigla: "RR", nome: "Roraima", regiao: { id: 1, sigla: "N", nome: "Norte" } },
      { id: 42, sigla: "SC", nome: "Santa Catarina", regiao: { id: 4, sigla: "S", nome: "Sul" } },
      { id: 35, sigla: "SP", nome: "São Paulo", regiao: { id: 3, sigla: "SE", nome: "Sudeste" } },
      { id: 28, sigla: "SE", nome: "Sergipe", regiao: { id: 2, sigla: "NE", nome: "Nordeste" } },
      { id: 17, sigla: "TO", nome: "Tocantins", regiao: { id: 1, sigla: "N", nome: "Norte" } },
    ];
  }
}
