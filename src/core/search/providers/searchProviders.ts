...
export const SEARCH_PROVIDER_BUCKET_ORDER = SEARCH_PROVIDERS.map(
  (provider) => provider.bucket,
) as readonly SearchBucket[];

export function hasSearchProvider(bucket: SearchBucket): boolean {
  return SEARCH_PROVIDERS.some((provider) => provider.bucket === bucket);
}
...
export function getSearchProviders(
  ...
  return SEARCH_PROVIDERS.filter(
    (provider) =>
      authorizedBuckets.has(provider.bucket) &&
