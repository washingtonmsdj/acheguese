...
/**
 * TerritoryModeSelector
 * 
 * Seletor com dois modos para usuarios cadastrados:
 * - "Meu Bairro": conteudo filtrado apenas pelo bairro do morador
 * - "Minha Cidade": conteudo da cidade inteira com filtros por bairro
 * 
 * Visitantes veem apenas modo cidade com filtros publicos.
 * O bairro ativo NUNCA muda para outro bairro que nao o do morador.
 */
...
interface TerritoryModeSelectorProps {
  compact?: boolean;
}

export function TerritoryModeSelector({
  compact = false,
}: TerritoryModeSelectorProps) {
  ...
  // SSOT: path canonico do territorio ativo
  ...
  // Detectar contexto da rota (apenas modulo)
  ...
  // Handler: selecionar outro territorio (visitantes ou explorar)
  const handleSelect = useCallback((path: string) => {
    let finalPath = path;
    if (routeContext.module) {
      finalPath = `/${routeContext.module}${path}`;
    }
    // Visitantes: sem modo. Cadastrados: forca modo cidade ao explorar
    if (hasHome) {
      setTerritoryMode('cidade');
    }
    navigate(finalPath);
  ...
  // Territorios disponiveis para explorar (excluindo bairro/cidade do usuario)
      ...
      <DialogContent className="sm:max-w-md max-h-[85vh] flex flex-col p-0 gap-0">
        <DialogHeader className="px-4 pt-4 pb-2">
          <DialogTitle className="text-base">
            {hasHome ? 'Escolha seu modo' : 'Onde voce esta?'}
          </DialogTitle>
        </DialogHeader>

        {/* Modo toggle para usuarios cadastrados */}
        {hasHome && (
        ...
        <ScrollArea className="flex-1 max-h-[60vh]">
          ...
          {/* Territorios disponiveis (quando nao busca) */}
          {!searchQuery && availableTerritories.length > 0 && (
            <div className="px-3 pb-4 space-y-3">
              <div className="flex items-center gap-3">
                <div className="flex-1 h-px bg-border" />
                <span className="text-[10px] text-muted-foreground uppercase tracking-wider">
                  {hasHome ? 'Explorar outros locais' : 'Territorios disponiveis'}
                </span>
                <div className="flex-1 h-px bg-border" />
              </div>
