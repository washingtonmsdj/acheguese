...
export class SettlementContextService {
  static calculateFinancialBreakdown(
    ...
    if (itemsTotal < 0 || deliveryFee < 0 || discountTotal < 0 || orderTotal < 0) {
      throw new Error("Breakdown financeiro invalido: valores nao podem ser negativos.");
    }
    ...
    if (platformFee !== null && platformFee < 0) {
      throw new Error("Breakdown financeiro invalido: platform_fee_amount nao pode ser negativo.");
    }

    if (courierAmount !== null && courierAmount < 0) {
      throw new Error("Breakdown financeiro invalido: courier_amount nao pode ser negativo.");
    }

    if (merchantNet !== null && merchantNet < 0) {
      throw new Error("Breakdown financeiro invalido: merchant_net_amount nao pode ser negativo.");
    }
    ...
    if (merchantNet !== null && merchantNet < 0) {
      throw new Error(
        "Breakdown financeiro invalido: merchant_net_amount calculado ficou negativo.",
      );
    }
  ...
  static buildSettlementContext(params: {
    ...
    const settlementReason =
      ...
      delivery_mode === DELIVERY_MODE.MERCHANT_OWN_FLEET
        ? "Modo atual: pagamento direto ao merchant e frota propria. Settlement apenas preparado."
        : "Modo marketplace preparado, mas execucao de split/payout ainda nao habilitada.";
  ...
  static assertSettlementExecutionEnabled(): never {
    throw new Error(
      "Execucao real de payout/split/settlement nao esta habilitada nesta fase.",
    );
  }
}
