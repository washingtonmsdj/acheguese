...
export type NeighborhoodStreamUrls = {
  feed: string;
  business: string;
  services: string;
  classifieds: string;
...
function formatBusinessMeta(rating: number | null | undefined, isVerified: boolean | null | undefined): string {
  const normalizedRating = Number.isFinite(rating) ? Number(rating) : 0;
  if (normalizedRating <= 0) return 'Empresa do bairro';

  const verificationLabel = isVerified ? 'verificada' : 'local';
  return `★ ${normalizedRating.toFixed(1).replace('.', ',')} • ${verificationLabel}`;
}

function formatServiceMeta(rating: number | null | undefined, priceRange: string | undefined): string {
  const normalizedRating = Number.isFinite(rating) ? Number(rating) : 0;
  const priceLabel = priceRange || 'A combinar';
  if (normalizedRating <= 0) return priceLabel;

  return `★ ${normalizedRating.toFixed(1).replace('.', ',')} • ${priceLabel}`;
}

function formatGastronomyMeta(rating: number | null | undefined): string {
  const normalizedRating = Number.isFinite(rating) ? Number(rating) : 0;
  if (normalizedRating <= 0) return 'Restaurante do bairro';

  return `★ ${normalizedRating.toFixed(1).replace('.', ',')} • Restaurante`;
}
...
export function buildNeighborhoodStreamItems({
  ...
  const businessItem: NeighborhoodStreamItem[] = businesses.slice(0, 4).map((business) => ({
    ...
    mediaFallback: Store,
    meta: formatBusinessMeta(business.rating, business.is_verified),
  ...
  const serviceItem: NeighborhoodStreamItem[] = services.slice(0, 4).map((service) => ({
    ...
    meta: formatServiceMeta(service.rating, service.price_range),
  ...
  const gastronomyItem: NeighborhoodStreamItem[] = gastronomyItems.slice(0, 4).map((restaurant) => ({
    ...
    meta: formatGastronomyMeta(restaurant.rating),
