...
function hasDistrictTerritorySegments(geoPath: string): boolean {
  return geoPath.split('/').filter(Boolean).length >= 4;
}
...
export class ClassifiedUrlService {
  ...
  static buildPublicUrl(ctx: ClassifiedPublicUrlInput): string | null {
    ...
    if (
      ctx.geographic_path &&
      hasDistrictTerritorySegments(ctx.geographic_path) &&
      ctx.category_slug &&
      ctx.subcategory_slug &&
      ctx.slug
    ) {
      try {
