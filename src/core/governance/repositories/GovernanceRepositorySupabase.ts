...
function toLocationVersion(row: LocationVersionRow): LocationVersion {
  return {
    ...row,
...
function toTerritoryChangeEvent(row: TerritoryChangeEventRow): TerritoryChangeEvent {
  return {
    ...row,
    processed_by: row.processed_by ?? null,
  } as TerritoryChangeEvent;
}
...
export class GovernanceRepositorySupabase implements IGovernanceRepository {
  ...
  async createLocationVersion(data: CreateLocationVersionInput): Promise<LocationVersion> {
    ...
    return toLocationVersion(version as LocationVersionRow);
  }
  ...
  async getActiveVersionForLocation(locationId: string): Promise<LocationVersion | null> {
    ...
    if (error) throw error;
    return data ? toLocationVersion(data as LocationVersionRow) : null;
  }
  ...
  async listVersionsForLocation(locationId: string): Promise<LocationVersion[]> {
    ...
    if (error) throw error;
    return (data ?? []).map((row) => toLocationVersion(row as LocationVersionRow));
  }
  ...
  async createTerritoryChangeEvent(data: CreateTerritoryChangeEventInput): Promise<TerritoryChangeEvent> {
    ...
    if (error) throw error;
    return toTerritoryChangeEvent(event as TerritoryChangeEventRow);
  }
  ...
  async listEventsForLocation(locationId: string): Promise<TerritoryChangeEvent[]> {
    ...
    if (error) throw error;
    return (data ?? []).map((row) =>
      toTerritoryChangeEvent(row as TerritoryChangeEventRow),
    );
  }
