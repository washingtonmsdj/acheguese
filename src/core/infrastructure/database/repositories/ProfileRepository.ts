...
export class ProfileRepository extends BaseRepository<Profile> {
  ...
  /**
   * Busca profiles com mais pontos (ranking)
   */
  async findTopByPoints(limit: number = 10): Promise<Profile[]> {
    try {
      ...
      if (error) {
        throw DatabaseError.fromSupabaseError(error, 'findTopByPoints', this.table);
      }
      ...
      throw new DatabaseError({
        code: DatabaseErrorCode.QUERY_ERROR,
        message: `Failed to find top profiles by points`,
        originalError: error,
        table: this.table,
        operation: 'findTopByPoints',
        context: { limit },
      });
    }
  }
   ...
   */
  async incrementPoints(id: string, points: number): Promise<Profile> {
    try {
      ...
      if (!profile) {
        throw new DatabaseError({
          ...
          table: this.table,
          operation: 'incrementPoints',
          context: { id },
        });
      }

      // Atualizar com novos pontos
      const newPoints = profile.pontos + points;
      return this.update(id, { pontos: newPoints } as Partial<Profile>);
    } catch (error) {
      if (error instanceof DatabaseError) throw error;
      throw new DatabaseError({
        code: DatabaseErrorCode.QUERY_ERROR,
        message: `Failed to increment points for profile: ${id}`,
        originalError: error,
        table: this.table,
        operation: 'incrementPoints',
        context: { id, points },
