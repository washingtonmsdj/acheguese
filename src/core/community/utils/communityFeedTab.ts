...
export const COMMUNITY_FEED_HEADER_FILTERS: readonly CommunityFeedHeaderFilterOption[] =
  RAW_COMMUNITY_FEED_HEADER_FILTERS.filter(({ id }) => isLaunchCommunityFeedChannelEnabled(id));
...
export function resolveCommunityFeedChannelFromTab(
  ...
  const channel = COMMUNITY_TAB_TO_CHANNEL[tab as CommunityDiscoveryTab] ?? "para_voce";
