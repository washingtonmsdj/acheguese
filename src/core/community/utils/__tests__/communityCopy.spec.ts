...
describe("communityCopy SSOT", () => {
  it("keeps critical feed labels stable", () => {
    expect(COMMUNITY_FEED_COPY.composerTitle).toBe("O que voce quer compartilhar com o bairro?");
    expect(COMMUNITY_FEED_COPY.headerFiltersAriaLabel).toBe("Filtros principais do feed");
    expect(COMMUNITY_FEED_COPY.sortAriaLabel).toBe("Ordenacao do feed");
  });

  it("keeps critical page labels stable", () => {
    expect(COMMUNITY_PAGE_COPY.loginRequiredTitle).toBe("Faca login para acessar a comunidade");
    expect(COMMUNITY_PAGE_COPY.rolloutBlockedTitle).toBe("Comunidade ainda nao liberada");
  ...
  it("keeps location scope labels stable", () => {
    ...
    expect(COMMUNITY_LOCATION_SCOPE_COPY.neighborhoodGroupLabel).toBe("Toda a area");
  ...
  it("keeps post card labels stable", () => {
    ...
    expect(COMMUNITY_POST_CARD_COPY.locationFallback).toBe("Localizacao nao informada");
