...
// Inlined contract reference to avoid Vite raw-import build issues
const contractText = `# Game Template Contract\n\nAll game code must live under src/games/_template/ with canonical directories:\nsystems/, entities/, ui/, state/, input/, audio/, spawn/, utils/, _derived/\n\nRules:\n1. All files must use .ts, .tsx, or .json extensions\n2. No code outside canonical directories\n3. Systems must implement update(deltaTime)\n4. State management via FSM with GameState enum\n5. UI must include StartScreen, HUD, GameOverScreen`;
...
export function applyCodeSemanticPatch(vfs: VFS, patch: CodeSemanticPatch): ApplyPatchResult {
  ...
  try {
    ...
    for (const op of patch.ops) {
      if (!op || typeof op !== "object" || typeof (op as Record<string, unknown>).op !== "string") {
      ...
      switch (op.op) {
        ...
        case "update_file":
        case "delete_file": {
          validateSinglePathOrThrow((op as Record<string, unknown>).path as string, patch.gameId);

          // Light semantic requirement: if we touch canonical dirs, require codeGame.ts update in same patch.
          const p = normalizePath((op as Record<string, unknown>).path as string);
        ...
        case "rename_file": {
          validateSinglePathOrThrow((op as Record<string, unknown>).from as string, patch.gameId);
          validateSinglePathOrThrow((op as Record<string, unknown>).to as string, patch.gameId);
          // Deterministic and safe rename: same directory only.
          if (dirname((op as Record<string, unknown>).from as string) !== dirname((op as Record<string, unknown>).to as string)) {
            throw new Error(`rename_file só pode renomear dentro do mesmo diretório: ${(op as Record<string, unknown>).from as string} -> ${(op as Record<string, unknown>).to as string}`);
          }
          break;
        }
        default:
          throw new Error(`Op desconhecida: ${(op as Record<string, unknown>).op as string}`);
      }

      if (op.op === "delete_file") {
        assertDoesNotDeleteMandatory((op as Record<string, unknown>).path as string, patch.gameId);
      }
      if (op.op === "rename_file") {
        assertDoesNotDeleteMandatory((op as Record<string, unknown>).from as string, patch.gameId);
      }
    }
    ...
    if (requiresRegistration.size) {
      ...
      const hasCodeGameUpdate = patch.ops.some((op) => op.op === "update_file" && normalizePath((op as Record<string, unknown>).path as string) === codeGamePath);
