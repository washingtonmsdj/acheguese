...
function extractThemes(spec: RuntimeSpec): string[] {
  ...
  const sprites = Object.values(spec.entities || {})
    .map((e) => (e as Record<string, unknown>).sprite)
    .filter(Boolean);
  ...
  for (const [theme, emojis] of Object.entries(themeMap)) {
    if (sprites.some(s => emojis.includes(s as string))) {
      themes.push(theme);
    }
  }
...
function extractDifficulty(spec: RuntimeSpec): 'easy' | 'normal' | 'hard' {
  ...
  const enemies = Object.values(spec.entities || {})
    .filter((e) => (e as Record<string, unknown>).damage || (e as Record<string, unknown>).aiType);
  
  if (enemies.length === 0) return 'normal';
  
  const avgHealth = enemies.reduce((sum: number, e) => sum + ((e as Record<string, unknown>).health as number || 50), 0) / enemies.length;
  const avgDamage = enemies.reduce((sum: number, e) => sum + ((e as Record<string, unknown>).damage as number || 10), 0) / enemies.length;
...
function extractSpeed(spec: RuntimeSpec): 'slower' | 'normal' | 'faster' {
  ...
  const avgSpeed = entities.reduce((sum: number, e) => sum + ((e as Record<string, unknown>).speed as number || 100), 0) / entities.length;
...
function extractBoss(spec: RuntimeSpec): boolean {
  ...
  return Object.values(spec.entities || {}).some((e) => {
    const entity = e as Record<string, unknown>;
    return entity.aiType === 'boss' || 
      entity.sprite === '👑' ||
      (entity.health && (entity.health as number) > 300);
  });
}
...
function extractEnemyTypes(spec: RuntimeSpec): GameGenome['enemyTypes'] {
  const enemies = Object.entries(spec.entities || {})
    .filter(([_, e]) => (e as Record<string, unknown>).damage || (e as Record<string, unknown>).aiType)
    .map(([key, e]) => {
      const entity = e as Record<string, unknown>;
      return {
        key,
        health: entity.health as number || 50,
        damage: entity.damage as number || 10,
        speed: entity.speed as number || 100,
        behavior: entity.aiType as string || 'chase'
      };
    });
...
function extractMechanics(spec: RuntimeSpec): string[] {
  ...
  // Shooting
  if (entities.some((e) => (e as Record<string, unknown>).weapon)) {
    mechanics.push('shooting');
  }
  
  // Collection
  if (entities.some((e) => (e as Record<string, unknown>).scoreValue || (e as Record<string, unknown>).healAmount)) {
    mechanics.push('collection');
  }
  
  // Movement
  if (entities.some((e) => (e as Record<string, unknown>).speed)) {
    mechanics.push('movement');
  }
  
  // Combat
  if (entities.some((e) => (e as Record<string, unknown>).damage)) {
  ...
  // Health
  if (entities.some((e) => (e as Record<string, unknown>).health)) {
