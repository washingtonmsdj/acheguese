...
export function validateRemix(remixedSpec: RuntimeSpec): RemixValidation {
  ...
  const hasEnemies = Object.values(remixedSpec.entities || {}).some(
    (e) => (e as Record<string, unknown>).damage || (e as Record<string, unknown>).aiType
  );
  const hasCollectibles = Object.values(remixedSpec.entities || {}).some(
    (e) => (e as Record<string, unknown>).scoreValue || (e as Record<string, unknown>).healAmount
  );
  ...
  Object.entries(remixedSpec.entities || {}).forEach(([key, entity]: [string, Record<string, unknown>]) => {
  ...
  Object.entries(remixedSpec.spawners || {}).forEach(([key, spawner]: [string, Record<string, unknown>]) => {
