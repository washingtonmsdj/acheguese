...
import { RACING_CONTRACT } from "@/lib/ordax/contracts/racing.contract";
...
function hasAnyEntity(spec: OrdaxSpec, keys: string[]): boolean {
  return keys.some((k) => hasEntity(spec, k));
}

function getPlayerProps(spec: OrdaxSpec): Record<string, unknown> {
  const player = (spec.scene?.entities ?? []).find(
    (e) => String(e.type).toLowerCase() === "player" || String(e.id).toLowerCase() === "player"
  );
  return (player?.props ?? {}) as Record<string, unknown>;
}
...
/**
 * Type-safe plan structure for validation
 */
...
export function validateRuntimeAgainstPlan(plan: unknown, runtimeSpec: OrdaxSpec): ValidateRuntimeAgainstPlanResult {
  ...
  const mustHave = (p.mustHave ?? {}) as PlanStructure['mustHave'];
  const lifecycle = (p.lifecycle ?? {}) as PlanStructure['lifecycle'];

  const planGameType = String(p.gameType ?? "").toLowerCase();
  const runtimeGameType = String((runtimeSpec as Record<string, unknown>).gameType ?? "").toLowerCase();
  // Regra: o plano aceito é a fonte de verdade. Não aplicamos contratos rígidos
  // (ex: racing) apenas porque o runtimeSpec sugeriu um gênero diferente.
  const isRacing = planGameType === "racing";
  ...
  if (isRacing) {
    const missingRacingEntities: string[] = [];
    if (!hasAnyEntity(runtimeSpec, ["player_vehicle", "vehicle", "car", "player"])) {
      missingRacingEntities.push("player_vehicle");
    }
    if (!hasAnyEntity(runtimeSpec, ["track", "road", "lane"])) {
      missingRacingEntities.push("track");
    }
    if (!hasAnyEntity(runtimeSpec, ["finish_line", "finish", "goal"])) {
      missingRacingEntities.push("finish_line");
    }

    if (missingRacingEntities.length) {
      issues.push({
        code: "MISSING_REQUIRED_ENTITY",
        severity: "error",
        message: `Racing exige entidades mínimas (${missingRacingEntities.join(", ")}), mas elas não existem na scene.entities.`,
      });
    }

    for (const sys of RACING_CONTRACT.requiredSystems) {
      if (!hasSystem(runtimeSpec, sys)) {
        issues.push({
          code: "MISSING_REQUIRED_SYSTEM",
          severity: "error",
          message: `Racing exige o system '${sys}', mas ele não existe no runtimeSpec.systems.`,
        });
      }
    }

    const shooterArtifacts: string[] = [];
    if (hasEntity(runtimeSpec, "enemy")) shooterArtifacts.push("enemy");
    if (hasEntity(runtimeSpec, "spawner")) shooterArtifacts.push("spawner");
    if (hasEntity(runtimeSpec, "bullet")) shooterArtifacts.push("bullet");
    if (hasSystem(runtimeSpec, "AISystem")) shooterArtifacts.push("AISystem");
    if (hasSystem(runtimeSpec, "SpawnerSystem")) shooterArtifacts.push("SpawnerSystem");
    const playerProps = getPlayerProps(runtimeSpec);
    if (typeof playerProps.fireRate === "number" || typeof playerProps.bulletSpeed === "number") {
      shooterArtifacts.push("player.shooterProps");
    }

    if (shooterArtifacts.length) {
      issues.push({
        code: "SEMANTIC_MISMATCH",
        severity: "error",
        message: `SEMANTIC_MISMATCH: Racing game using shooter runtime (${shooterArtifacts.join(", ")})`,
      });
    }

    const hasRacingControlProps =
      typeof playerProps.acceleration === "number" ||
      typeof playerProps.maxSpeed === "number" ||
      typeof playerProps.turnRate === "number" ||
      typeof playerProps.steering === "number" ||
      typeof playerProps.brakeForce === "number";

    if (!hasRacingControlProps) {
      issues.push({
        code: "ENGINE_CONTRACT_VIOLATION",
        severity: "error",
        message: "Racing exige controle de veículo (aceleração/velocidade/esterço), mas props mínimas não foram encontradas no player.",
      });
    }
  }
  ...
  const baseStates = new Set(["start", "playing", "gameover"]);
  const baseTransitions = new Set(["start->playing", "playing->gameover", "gameover->restart"]);
  const baseUI = new Set(["hud", "gameover_screen"]);
  const baseControls = new Set(["start_game", "restart_game"]);
  const hasLifecycleExtras =
    requiredStates.some((s) => !baseStates.has(String(s))) ||
    requiredTransitions.some((t) => !baseTransitions.has(String(t))) ||
    requiredUI.some((u) => !baseUI.has(String(u))) ||
    requiredControls.some((c) => !baseControls.has(String(c)));

  if (hasLifecycleExtras) {
    gaps.push({
      area: "Lifecycle",
      limitation:
  ...
  if (gaps.length) {
    issues.push({
      code: "ENGINE_GAP",
      severity: "error",
      message: `ENGINE_GAP: Plano requer validações não representáveis no runtimeSpec atual (${gaps
        .map((g) => g.area)
        .join(", ")}).`,
    });
  }
