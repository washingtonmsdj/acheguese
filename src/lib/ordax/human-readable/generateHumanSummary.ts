...
function detectGenre(gameType: string | undefined, systems: string[], entities: Record<string, unknown>[]): string {
  if (gameType === 'topdown') {
...
function detectObjective(gameType: string | undefined, entities: Record<string, unknown>[]): string {
  if (gameType === 'topdown') {
    const hasSpawner = entities.some(e => e.type === 'spawner');
    if (hasSpawner) {
...
function detectControls(entities: Record<string, unknown>[]): { movement: string; actions: string[] } {
...
function detectMechanics(systems: string[], entities: Record<string, unknown>[]): string[] {
...
function detectEnemies(entities: Record<string, unknown>[]): { types: string[]; behaviors: string[] } {
...
function detectProgression(entities: Record<string, unknown>[], systems: string[]): string {
  const hasSpawner = entities.some(e => e.type === 'spawner');
...
function detectDefeat(entities: Record<string, unknown>[]): string {
...
function detectVisual(spec: OrdaxSpec): { theme: string; style: string } {
  ...
  if (background?.layers) {
    const hasStarfield = background.layers.some((l: Record<string, unknown>) => l.type === 'starfield');
    const hasNebula = background.layers.some((l: Record<string, unknown>) => l.type === 'nebula');
