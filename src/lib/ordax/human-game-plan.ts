...
export function buildHumanGamePlanSections(input: BuildHumanPlanInput): HumanSection[] {
  ...
  const planDiff = (input.planDiff ?? {}) as Record<string, unknown>;
  ...
  const lifecycle = (plan.lifecycle ?? {}) as Record<string, unknown>;
  ...
  const lifecycleDiff = (planDiff?.lifecycle ?? {}) as Record<string, unknown>;
