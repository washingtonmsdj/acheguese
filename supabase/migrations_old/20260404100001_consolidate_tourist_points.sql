-- ============================================================================
-- CONSOLIDAÃ‡ÃƒO: tourist_points como tabela canÃ´nica
--
-- DECISÃƒO: tourist_points Ã© a tabela canÃ´nica porque:
-- 1. JÃ¡ tem coluna point (PostGIS) para busca espacial
-- 2. JÃ¡ tem location_id e address_id (FK canÃ´nicas)
...
BEGIN
  ...
  SELECT
    ...
    'outro' AS category,                                  -- tourist_points_v2 nÃ£o tem category
    ARRAY[]::TEXT[] AS tags,                              -- tourist_points_v2 nÃ£o tem tags
    ...
    NULL AS latitude,                                     -- tourist_points_v2 nÃ£o tem coordenadas diretas
    NULL AS longitude,                                    -- tourist_points_v2 nÃ£o tem coordenadas diretas
    (
      SELECT url FROM tourist_point_media
      WHERE tourist_point_id = tp2.id AND is_cover = true
      LIMIT 1
    ) AS photo_url,                                       -- buscar cover da tabela de mÃ­dia
    (
      SELECT ARRAY_AGG(url ORDER BY display_order)
      FROM tourist_point_media
      WHERE tourist_point_id = tp2.id
    ) AS gallery_urls,                                    -- buscar todas as mÃ­dias
    'ðŸ›ï¸' AS icon_emoji,                                   -- emoji padrÃ£o
    ...
    NULL AS phone,                                        -- tourist_points_v2 nÃ£o tem phone
    CASE
      WHEN tp2.accessibility_notes IS NOT NULL AND tp2.accessibility_notes != '' THEN true
      ELSE false
    END AS accessibility,
    false AS has_parking,                                 -- tourist_points_v2 nÃ£o tem has_parking
    false AS has_restaurant,                              -- tourist_points_v2 nÃ£o tem has_restaurant
    false AS has_guide,                                   -- tourist_points_v2 nÃ£o tem has_guide
    tp2.is_featured,
    0 AS display_order,                                   -- tourist_points_v2 nÃ£o tem display_order
    0 AS rating,                                          -- tourist_points_v2 nÃ£o tem rating
    0 AS total_reviews,                                   -- tourist_points_v2 nÃ£o tem total_reviews
    CASE
  ...
  WHERE NOT EXISTS (
    -- Evitar duplicatas: nÃ£o inserir se jÃ¡ existe com mesmo slug e location_id
  ...
  RAISE NOTICE 'ConsolidaÃ§Ã£o concluÃ­da: % registros migrados de tourist_points_v2 para tourist_points', v_count;
...
COMMENT ON TABLE tourist_points IS 'Tabela canÃ´nica de pontos turÃ­sticos. Usa location_id (FK locations) e address_id (FK addresses) como modelo territorial. Coluna point (PostGIS) para busca espacial.';

-- Nota: tourist_points_v2 pode ser mantida para referÃªncia ou removida em migration futura
-- DecisÃ£o de remoÃ§Ã£o deve ser tomada apÃ³s validaÃ§Ã£o completa da migraÃ§Ã£o
