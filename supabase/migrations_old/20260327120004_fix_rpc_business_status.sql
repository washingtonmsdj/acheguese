-- ============================================================================
-- FIX: create_profile_with_extension - Corrigir business status
-- ============================================================================
-- PROBLEMA: business_data tem constraint que exige location_id quando status='active'
-- SOLUÇÃO: Setar status='pending' por padrão (draft mode)
-- ============================================================================
...
BEGIN
  ...
  -- Criar extensão
  IF p_profile_type = 'business' THEN
    -- FIX: Setar status='pending' para evitar constraint de location_id
    INSERT INTO business_data (
      profile_id, business_name, legal_name, cnpj, company_type, industry, status
    ) VALUES (
      v_profile_id,
      ...
      'pending'  -- Draft mode - não exige location_id
    );
...
COMMENT ON FUNCTION create_profile_with_extension IS 'Multi-perfil: criar perfil com extensão (FIXED: business status=pending)';
