...
export function validateRuntimeAgainstProfile(
  ...
  const entityTypeMap = new Map<string, unknown[]>();
  ...
  for (const requiredEntity of profile.requiredEntities) {
    ...
    const sampleEntity = entitiesOfType[0] as Record<string, unknown>;
    const entityProps = (sampleEntity.props as Record<string, unknown>) || {};
