...
async function handleCheckMfaRequired(
  supabaseAdmin: SupabaseClient,
  auth: UserAuthResult,
) {
  const policy = await evaluateUserMfaPolicy(
    supabaseAdmin,
    auth.userId,
    auth.token,
  );
  ...
  return {
    ...
    hasVerifiedFactor: policy.hasVerifiedFactor,
    currentLevel: policy.currentLevel,
    gracePeriodExpiresAt: policy.gracePeriodExpiresAt,
  };
}
...
async function handleRevokeAllSessions(
  ...
  const exceptCurrent =
    params.exceptCurrent !== false && params.except_current !== false;
...
async function dispatchAction(
  ...
  switch (action) {
    ...
    case "checkMfaRequired":
      return handleCheckMfaRequired(supabaseAdmin, auth);
