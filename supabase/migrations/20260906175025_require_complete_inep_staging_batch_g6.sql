ALTER TABLE private.education_public_import_batches
  DROP CONSTRAINT IF EXISTS education_public_import_batches_target_row_count_chk;

ALTER TABLE private.education_public_import_batches
  ADD CONSTRAINT education_public_import_batches_target_row_count_chk
  CHECK (
    COALESCE(manifest#>>'{counts,target_municipality_rows}', '')
      ~ '^[1-9][0-9]{0,8}$'
  );

COMMENT ON CONSTRAINT education_public_import_batches_target_row_count_chk
ON private.education_public_import_batches IS
  'Requires a positive bounded target_municipality_rows manifest count before staging validation.';
...
BEGIN
  SELECT b.target_city_id,
         ...
         b.source_year,
         b.status,
         (b.manifest#>>'{counts,target_municipality_rows}')::integer,
         l.geographic_path
  ...
  IF v_total <> v_expected_rows THEN
    RAISE EXCEPTION 'education_import_batch_row_count_mismatch'
      USING
        ERRCODE = '23514',
        DETAIL = format(
          'expected_target_rows=%s staged_rows=%s',
          v_expected_rows,
          v_total
        );
  END IF;
...
COMMENT ON FUNCTION private.validate_public_education_import_batch(uuid) IS
  'Validates only complete INEP staging batches whose staged row count matches the adapter manifest, then applies natural-key, source-binding and canonical conflict checks. Never mutates public Education/Profile/Business data.';
