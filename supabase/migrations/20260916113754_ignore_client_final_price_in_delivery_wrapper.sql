...
COMMENT ON FUNCTION public.mobility_transition_delivery_state_atomic(
  uuid, text, text, text, text, jsonb, numeric, jsonb
) IS 'G70 service-only delivery boundary. p_final_price is compatibility-only and ignored; price comes from persisted quote/ride data; confirm_delivery records delivered and completed atomically.';
