...
BEGIN
  ...
  IF NOT has_function_privilege('service_role', v_helper_oid, 'EXECUTE')
     OR NOT has_function_privilege('service_role', v_status_oid, 'EXECUTE')
  ...
  FOREACH v_function_oid IN ARRAY ARRAY[
    to_regprocedure('private.auth_account_operational()')::OID,
    to_regprocedure('private.current_active_profile_id()')::OID,
    to_regprocedure('private.auth_owns_usable_profile(uuid)')::OID,
    to_regprocedure('private.auth_can_access_profile(uuid)')::OID,
    to_regprocedure('private.can_manage_profile(uuid)')::OID,
    to_regprocedure('private.auth_participates_community_direct_thread(uuid)')::OID
  ]
  LOOP
    SELECT prosrc INTO v_definition FROM pg_proc WHERE oid = v_function_oid;

    IF v_definition NOT LIKE '%private.auth_account_operational()%'
       AND v_function_oid <> v_helper_oid THEN
      RAISE EXCEPTION 'operational account helper is not wired into function %', v_function_oid::regprocedure;
    END IF;

    IF v_definition LIKE '%auth.role()%' THEN
      RAISE EXCEPTION 'deprecated auth.role() remains in pending-deletion authority function %', v_function_oid::regprocedure;
    END IF;
  END LOOP;
END;
$$;
