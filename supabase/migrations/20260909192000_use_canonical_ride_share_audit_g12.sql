-- G12 follow-up: keep the existing ride_shares audit trigger as the single audit producer.
--
-- The server-owned creation command must not duplicate the canonical
-- trg_audit_ride_share_insert side effect.
...
BEGIN
  ...
  WHERE tg.tgrelid = 'public.ride_shares'::regclass
    AND tg.tgname = 'trg_audit_ride_share_insert'
    AND NOT tg.tgisinternal;

  IF v_audit_trigger_count <> 1 THEN
    RAISE EXCEPTION 'canonical ride-share audit trigger missing';
  END IF;
END;
$$;
