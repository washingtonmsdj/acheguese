REVOKE ALL PRIVILEGES ON TABLE public.operational_verifications FROM anon, authenticated;

DROP POLICY IF EXISTS "operational_verifications_insert_by_requester" ON public.operational_verifications;
DROP POLICY IF EXISTS "operational_verifications_select_by_participant" ON public.operational_verifications;
DROP POLICY IF EXISTS "operational_verifications_update_by_participant" ON public.operational_verifications;
...
BEGIN
  ...
  IF NOT has_function_privilege('authenticated','public.create_operational_pin_verification(uuid,boolean,text,text)','EXECUTE')
     OR NOT has_function_privilege('authenticated','public.get_operational_verification_status(uuid)','EXECUTE')
     OR NOT has_function_privilege('authenticated','public.verify_operational_pin(uuid,text)','EXECUTE') THEN
