...
BEGIN
  FOR r IN
    SELECT *
    FROM (VALUES
      ('site_settings'::text, 'idx_site_settings_key'::text, 'site_settings_key_key'::text),
      ('notification_preferences'::text, 'idx_notification_preferences_user'::text, 'notification_preferences_user_id_key'::text)
