-- Stage 1 of the Gate 7 PIN hardening.
--
-- Browser code must not receive pin_hash or mutate verification state directly.
-- Add server-authoritative RPCs first so the frontend can migrate without a
-- production outage. Direct table authority is removed in a follow-up migration
-- only after the new client path is deployed.
...
BEGIN
  ...
  IF p_verification_type <> 'pin' THEN
    RAISE EXCEPTION 'unsupported_operational_verification_type'
      USING ERRCODE = '22023';
  END IF;

  IF p_is_required AND p_required_by NOT IN (
    'admin', 'passenger', 'driver', 'sender', 'operation'
  ) THEN
    RAISE EXCEPTION 'invalid_operational_verification_required_by'
      USING ERRCODE = '22023';
  END IF;
  ...
  -- Verification creation belongs to the requester side of the ride. Platform
  -- admins retain support authority, but arbitrary ride participants cannot
  -- impose/replace a PIN requirement.
  ...
  WHERE verification.ride_id = p_ride_id
    AND verification.verification_type = p_verification_type
  FOR UPDATE;
  ...
  -- Idempotent retry: never expose/recreate a plaintext PIN for an already
  -- pending/verified verification. A dedicated rotation flow can be added later.
  IF FOUND AND v_existing.status IN ('pending', 'verified') THEN
    RETURN jsonb_build_object(
      'verification_id', v_existing.id,
      'pin', NULL,
      'expires_at', v_existing.pin_expires_at
    );
  END IF;
  ...
  RETURN jsonb_build_object(
    'verification_id', v_result.id,
    'pin', v_pin,
    'expires_at', v_result.pin_expires_at
  );
END;
$function$;
...
BEGIN
  ...
  -- Deliberately omit pin_hash. Browser clients never need verifier material.
  RETURN jsonb_build_object(
    'id', v_verification.id,
    'ride_id', v_verification.ride_id,
    'verification_type', v_verification.verification_type,
    'is_required', v_verification.is_required,
    'required_by', v_verification.required_by,
    'required_at', v_verification.required_at,
    'status', v_verification.status,
    'pin_generated_at', v_verification.pin_generated_at,
    'pin_expires_at', v_verification.pin_expires_at,
    'verified_at', v_verification.verified_at,
    'verified_by', v_verification.verified_by,
    'verification_attempts', COALESCE(v_verification.verification_attempts, 0),
    'last_attempt_at', v_verification.last_attempt_at,
...
BEGIN
  ...
  IF NOT FOUND
     OR (
       v_actor_profile_id IS DISTINCT FROM v_ride.passenger_profile_id
       AND v_actor_profile_id IS DISTINCT FROM v_ride.driver_profile_id
     ) THEN
    RAISE EXCEPTION 'ride_participant_required' USING ERRCODE = '42501';
  END IF;
  ...
  IF v_verification.status = 'verified' THEN
    RETURN jsonb_build_object(
      ...
      'attempts_remaining', GREATEST(0, 5 - COALESCE(v_verification.verification_attempts, 0))
    );
  END IF;
  ...
  IF v_verification.pin_expires_at IS NULL OR v_verification.pin_expires_at <= v_now THEN
    UPDATE public.operational_verifications
    SET status = 'failed',
        updated_at = v_now
  ...
  UPDATE public.operational_verifications
  SET status = CASE WHEN v_attempts_remaining = 0 THEN 'failed' ELSE 'pending' END,
      verification_attempts = v_attempts,
      last_attempt_at = v_now,
      updated_at = v_now
  WHERE id = v_verification.id;
...
COMMENT ON FUNCTION public.create_operational_pin_verification(uuid, boolean, text, text) IS
  'Server-authoritative Gate 7 PIN creation. Generates the PIN in Postgres and returns plaintext only on first creation/reset.';
COMMENT ON FUNCTION public.get_operational_verification_status(uuid) IS
  'Sanitized Gate 7 verification status for authorized ride participants; never returns pin_hash.';
COMMENT ON FUNCTION public.verify_operational_pin(uuid, text) IS
  'Server-authoritative Gate 7 PIN verification with participant authorization, expiry and five-attempt enforcement.';
