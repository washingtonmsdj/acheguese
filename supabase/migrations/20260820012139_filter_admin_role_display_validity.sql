-- Keep administrative role displays aligned with the canonical authorization
-- contract: a current role must be active, unrevoked, and unexpired.
-- This changes presentation/query semantics only; it does not mutate user_roles.
    ...
    CASE
      ...
      ELSE COALESCE((
        ...
        WHERE role_record.user_id = page_row.user_id
          ...
          AND (role_record.expires_at IS NULL OR role_record.expires_at > now())
...
DECLARE
  v_definition TEXT;
BEGIN
  SELECT pg_get_functiondef('public.admin_list_user_account_contexts(integer,integer,text)'::regprocedure)
    INTO v_definition;

  IF position('role_record.is_active IS TRUE' in v_definition) = 0 THEN
    RAISE EXCEPTION 'postcondition failed: admin list RPC does not require active roles';
  END IF;
  IF position('role_record.revoked_at IS NULL' in v_definition) = 0 THEN
    RAISE EXCEPTION 'postcondition failed: admin list RPC does not reject revoked roles';
  END IF;
  IF position('role_record.expires_at IS NULL' in v_definition) = 0
     OR position('role_record.expires_at > now()' in v_definition) = 0 THEN
    RAISE EXCEPTION 'postcondition failed: admin list RPC does not reject expired roles';
  END IF;
END;
$$;
