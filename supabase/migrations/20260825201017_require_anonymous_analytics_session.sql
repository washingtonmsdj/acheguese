-- Close the anonymous analytics rate-limit bypass where callers could omit
-- session_id and therefore avoid both session- and authenticated-user counters.
-- Authenticated callers may still omit a session because they are rate-limited
-- by auth.uid(); service-role producers keep their trusted server contract.
...
do $$
declare
  v_oid oid := 'public.track_analytics_event(text,uuid,public.analytics_event_type,public.analytics_event_source,uuid,text,inet,text,text,numeric,numeric,jsonb)'::regprocedure;
  v_definition text;
begin
  select pg_get_functiondef(v_oid) into v_definition;

  if v_definition not ilike '%analytics_session_required%' then
    raise exception 'analytics session hardening postcondition failed';
  end if;
  if not has_function_privilege('anon', v_oid, 'EXECUTE')
     or not has_function_privilege('authenticated', v_oid, 'EXECUTE')
     or not has_function_privilege('service_role', v_oid, 'EXECUTE') then
    raise exception 'analytics session hardening grants missing';
  end if;
end
$$;
