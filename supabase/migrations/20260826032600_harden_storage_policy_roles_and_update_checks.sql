...
-- tryon: bucket remains public for generated-image delivery; object API writes
-- and owner listings are authenticated-only.
...
BEGIN
  SELECT count(*)
  INTO v_public_mutation_policies
  FROM pg_policies
  WHERE schemaname = 'storage'
    AND tablename = 'objects'
    AND cmd IN ('INSERT', 'UPDATE', 'DELETE')
    AND (
      qual ILIKE ANY (ARRAY[
        '%bucket_id = ''avatars''%',
        '%bucket_id = ''business_images''%',
        '%bucket_id = ''classified_images''%',
        '%bucket_id = ''event_images''%',
  ...
  IF v_public_mutation_policies <> 0 THEN
    RAISE EXCEPTION 'public/anon storage mutation policies remain on hardened buckets';
  END IF;

  SELECT count(*)
  INTO v_missing_update_checks
  FROM pg_policies
  WHERE schemaname = 'storage'
    AND tablename = 'objects'
    AND policyname IN (
      'Users can update their own avatar',
      'Users can update their own business images',
      'Users can update their own event images',
      'ai-images owner update',
      'tryon storage user update',
      'public-assets_update_admin'
    )
    AND with_check IS NULL;

  IF v_missing_update_checks <> 0 THEN
    RAISE EXCEPTION 'storage UPDATE policy missing WITH CHECK';
  END IF;
END
$verify$;
