-- ============================================================================
-- Fix public snapshot RPCs to handle district placeholder "_"
-- ============================================================================
-- When district is "_", it means "city level only, no specific district"
-- The geographic_path should be /br/state/city (without district segment)
-- ============================================================================
...
DECLARE
  v_geo_path TEXT;
  v_requested_business_url TEXT;
  v_requested_gastronomy_url TEXT;
  v_row RECORD;
...
BEGIN
  -- Handle district placeholder: "_" means city-level only
  IF p_district = '_' THEN
    v_geo_path := '/br/' || lower(trim(p_state)) || '/' || lower(trim(p_city));
    v_requested_business_url := '/empresas/' || lower(trim(p_state)) || '/' || lower(trim(p_city)) || '/' || p_slug;
    v_requested_gastronomy_url := '/gastronomia/' || lower(trim(p_state)) || '/' || lower(trim(p_city)) || '/' || p_slug;
  ELSE
    v_geo_path := '/br/' || lower(trim(p_state)) || '/' || lower(trim(p_city)) || '/' || lower(trim(p_district));
    v_requested_business_url := '/empresas/' || lower(trim(p_state)) || '/' || lower(trim(p_city)) || '/' || lower(trim(p_district)) || '/' || p_slug;
    v_requested_gastronomy_url := '/gastronomia/' || lower(trim(p_state)) || '/' || lower(trim(p_city)) || '/' || lower(trim(p_district)) || '/' || p_slug;
  END IF;
  ...
  -- Build URLs based on whether district is placeholder or real
  IF p_district = '_' THEN
    v_business_url := '/empresas/' || lower(trim(p_state)) || '/' || lower(trim(p_city)) || '/' || v_row.slug;
    v_gastronomy_url := '/gastronomia/' || lower(trim(p_state)) || '/' || lower(trim(p_city)) || '/' || v_row.slug;
  ELSE
