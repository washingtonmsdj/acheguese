...
AS $function$
  SELECT CASE
    WHEN jsonb_typeof(p_raw_record) <> 'object' THEN NULL
    WHEN EXISTS (
      SELECT 1
      FROM jsonb_each(p_raw_record) AS item(key, value)
    ...
    ELSE encode(
      extensions.digest(
        COALESCE(
          (
            SELECT string_agg(
              octet_length(item.key)::text || ':' || item.key || ':' ||
              octet_length(item.value)::text || ':' || item.value,
              E'\n'
              ORDER BY item.key COLLATE "C"
            )
            FROM jsonb_each_text(p_raw_record) AS item(key, value)
          ),
          ''
        ),
        'sha256'
...
REVOKE ALL ON FUNCTION private.education_inep_raw_record_sha256(jsonb)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION private.education_inep_raw_record_sha256(jsonb)
  TO service_role;

COMMENT ON FUNCTION private.education_inep_raw_record_sha256(jsonb) IS
  'Computes the canonical INEP raw-record SHA-256 used by adapter v5: UTF-8 byte-length-prefixed key/value pairs ordered by C collation. Returns NULL for non-string raw values.';

ALTER TABLE private.education_public_import_batches
  DROP CONSTRAINT IF EXISTS education_public_import_batches_record_hash_contract_chk;

ALTER TABLE private.education_public_import_batches
  ADD CONSTRAINT education_public_import_batches_record_hash_contract_chk
  CHECK (
    COALESCE(manifest#>>'{source,record_hash_contract}', '')
      = 'acheguese.inep-raw-record-sha256/1'
  );

COMMENT ON CONSTRAINT education_public_import_batches_record_hash_contract_chk
ON private.education_public_import_batches IS
  'Requires the batch manifest to declare the canonical raw-record SHA-256 contract used by the validator.';
  ...
  calculated AS (
    SELECT
      ...
      array_remove(ARRAY[
        ...
        END,
        CASE
          WHEN r.source_record_sha256 IS NULL
            OR lower(btrim(r.source_record_sha256)) !~ '^[0-9a-f]{64}$'
          THEN 'missing_or_invalid_source_record_sha256'
        END,
        CASE
          WHEN EXISTS (
            SELECT 1
            FROM jsonb_each(r.raw_record) AS raw_item(key, value)
            WHERE jsonb_typeof(raw_item.value) <> 'string'
          )
          THEN 'raw_record_non_string_value'
        END,
        CASE
          WHEN private.education_inep_raw_record_sha256(r.raw_record)
            IS DISTINCT FROM lower(btrim(r.source_record_sha256))
          THEN 'raw_record_sha256_mismatch'
        END,
        CASE
          WHEN (
            CASE
...
COMMENT ON FUNCTION private.validate_public_education_import_batch(uuid) IS
  'Validates INEP natural key, target municipality, dependency/network, active status, canonical raw-record hash binding, normalized-to-raw source binding, and existing canonical conflicts. Never mutates public Education/Profile/Business data.';
