...
COMMENT ON FUNCTION public.mobility_transition_ride_state_atomic(
  ...
  'Service-role-only atomic ride transition command. Locks the ride, validates the canonical state machine and ride mode, updates timestamps, invalidates open offers on terminal states, and writes ride_state_audit in the same transaction.';
