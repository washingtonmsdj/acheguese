CREATE UNIQUE INDEX IF NOT EXISTS education_profiles_school_inep_code_uidx
  ON public.education_profiles (school_inep_code)
...
CREATE TABLE IF NOT EXISTS private.education_public_import_batches (
  ...
  source_family text NOT NULL DEFAULT 'inep_censo_escolar'
    CHECK (source_family = 'inep_censo_escolar'),
  source_year integer NOT NULL CHECK (source_year BETWEEN 2007 AND 2100),
  source_landing_url text NOT NULL CHECK (
    char_length(source_landing_url) <= 2000
    AND source_landing_url ~* '^https://www\.gov\.br/inep/'
  ),
  source_archive_url text NOT NULL CHECK (
    char_length(source_archive_url) <= 2000
    AND source_archive_url ~* '^https://download\.inep\.gov\.br/'
  ),
  source_archive_sha256 text NOT NULL CHECK (
    source_archive_sha256 ~ '^[0-9a-f]{64}$'
  ),
  source_file_name text NOT NULL CHECK (
    char_length(source_file_name) BETWEEN 1 AND 255
  ),
  source_page_updated_at timestamptz,
  target_city_id uuid NOT NULL
    REFERENCES public.locations(id) ON DELETE RESTRICT,
  target_city_ibge_code text NOT NULL CHECK (
    target_city_ibge_code ~ '^[0-9]{7}$'
  ),
  parser_version text NOT NULL CHECK (
    char_length(parser_version) BETWEEN 1 AND 100
    AND parser_version ~ '^[A-Za-z0-9._/-]+$'
  ),
...
CREATE TABLE IF NOT EXISTS private.education_public_import_rows (
  ...
  batch_id uuid NOT NULL
    REFERENCES private.education_public_import_batches(id) ON DELETE CASCADE,
  source_row_number integer NOT NULL CHECK (source_row_number > 0),
  inep_code text,
  school_name text,
  uf_ibge_code text,
  municipality_ibge_code text,
  administrative_dependency_code text,
  ...
  source_record_sha256 text CHECK (
    source_record_sha256 IS NULL
    OR source_record_sha256 ~ '^[0-9a-f]{64}$'
  ),
  raw_record jsonb NOT NULL CHECK (
    jsonb_typeof(raw_record) = 'object'
    AND octet_length(raw_record::text) <= 131072
  ),
  normalized_school_network text,
  validation_status text NOT NULL DEFAULT 'pending' CHECK (
    validation_status IN ('pending','valid','excluded','invalid')
  ),
  validation_errors text[] NOT NULL DEFAULT '{}'::text[],
  candidate_action text NOT NULL DEFAULT 'none' CHECK (
    candidate_action IN ('none','insert','update')
  ),
  existing_education_profile_id uuid
    REFERENCES public.education_profiles(id) ON DELETE SET NULL,
  existing_business_data_id uuid
    REFERENCES public.business_data(id) ON DELETE SET NULL,
  staged_at timestamptz NOT NULL DEFAULT now(),
  validated_at timestamptz,
  UNIQUE (batch_id, source_row_number)
);

CREATE INDEX IF NOT EXISTS education_public_import_rows_batch_inep_idx
  ON private.education_public_import_rows (batch_id, inep_code);
CREATE INDEX IF NOT EXISTS education_public_import_rows_batch_status_idx
  ON private.education_public_import_rows (batch_id, validation_status);
...
REVOKE ALL ON TABLE private.education_public_import_rows
  FROM PUBLIC, anon, authenticated;
GRANT SELECT ON TABLE private.education_public_import_batches TO service_role;
GRANT SELECT ON TABLE private.education_public_import_rows TO service_role;
...
BEGIN
  IF COALESCE(auth.role(), '') <> 'service_role' THEN
    RAISE EXCEPTION 'education_import_service_role_required'
      USING ERRCODE = '42501';
  END IF;
  ...
  IF jsonb_typeof(COALESCE(p_manifest, '{}'::jsonb)) <> 'object'
     OR octet_length(COALESCE(p_manifest, '{}'::jsonb)::text) > 65536 THEN
