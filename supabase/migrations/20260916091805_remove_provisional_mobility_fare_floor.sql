-- Pricing values are still provisional. Atomic mobility commands must not
-- encode a fictitious commercial floor. They only reject non-positive input;
-- approved commercial policy remains owned by the Pricing SSOT.
...
CREATE OR REPLACE FUNCTION public.mobility_create_ride_atomic(
  ...
  p_departure_time timestamp with time zone DEFAULT NULL::timestamp with time zone
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_suggested_price IS NOT NULL AND p_suggested_price <= 0 THEN
    RAISE EXCEPTION 'suggested price must be positive' USING ERRCODE = '22023';
  END IF;
