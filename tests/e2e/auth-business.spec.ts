...
const EXISTING_LOGIN_PASSWORD =
  process.env.TEST_DRIVER_PASSWORD || process.env.E2E_USER_PASSWORD || null;
...
async function waitForProfileMember(
  profileId: string,
  userId: string,
): Promise<{ role: string }> {
  if (!admin) {
...
async function selectByPreferredOrFirst(
  ...
  if (required) {
    await expect(
      trigger,
      `Trigger nao encontrado: ${triggerSelector}`,
...
async function fillCadastroTerritory(page: Page) {
  ...
  await clickFirstEnabledOptionInCombobox(page, comboboxes.nth(0), [
    /bahia/i,
    /^ba$/i,
  ]);
  await clickFirstEnabledOptionInCombobox(page, comboboxes.nth(1), [
    /salvador/i,
  ]);
...
async function fillBusinessTerritory(page: Page) {
  ...
  if ((await legacyStateTrigger.count()) > 0) {
    ...
    await selectFirstAvailableOption(page, "#territorial-neighborhood", {
      required: false,
    });
    return;
  }
...
async function clickBusinessCreate(page: Page) {
  await page
    .getByRole("button", { name: "Criar empresa" })
    .evaluate((element) => {
      (element as HTMLButtonElement).click();
    });
}
...
test.describe.serial("Auth and business flow", () => {
  ...
  test("cadastro envia para confirmacao e cria identidade pessoal", async ({
    ...
    await expect(
      page.getByRole("heading", { name: /Onde você mora/i }),
    ).toBeVisible({
      timeout: 15000,
    });
    ...
    if (signupEmailInfraAvailable === false) {
      ...
      await expect(page.getByText(/Erro ao criar conta/i).first()).toBeVisible({
        timeout: 15_000,
      });
      await expect(
        page
    ...
    await expect(
      page.getByText(/Confirme seu email|Confirme seu e-mail/i),
    ).toBeVisible();
