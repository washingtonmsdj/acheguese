...
async function ensureBusinessProfileForE2EUser(): Promise<{
  ...
  let { data: existingBusinessProfile } = await client
    .from('profiles')
    .select('id')
  ...
  if (!existingBusinessProfile?.id) {
    ...
    const refreshed = await client
      ...
      .limit(1)
      .maybeSingle();
    existingBusinessProfile = refreshed.data ?? null;
  }
  ...
  if (existingBusinessProfile?.id) {
    ...
    const existingBusinessData = await client
      .from('business_data')
      .select('id, category')
...
async function createOrderFixture(): Promise<string | null> {
  ...
  const rpcResult = await client.rpc('delivery_create_order', {
  ...
  if (!orderId) {
    const existingOrder = await client
      .from('orders')
      .select('id')
      .eq('merchant_profile_id', BOOTSTRAP_BUSINESS_ID)
      .order('created_at', { ascending: false })
      .limit(1)
      .maybeSingle();
    orderId = existingOrder.data?.id ?? null;
  }
...
async function resolveBusinessId(page: Page) {
  ...
  const href = await page.evaluate(() => {
    const anchors = Array.from(document.querySelectorAll('a[href^="/central/empresas/"]'));
...
test.describe('Gastronomia operacional autenticada', () => {
  ...
  test('loja gerencia disponibilidade, estoque e esgotamento de item pelo cardapio canonico', async ({
    ...
    if (hasMenuHeading) {
      ...
      const soldOutAction = page.getByRole('button', { name: /marcar esgotado/i }).first();
  ...
  test('loja acessa dashboard e rota de pedido sem travar suspense global', async ({ page }) => {
    await loginAsUser(page);
    const businessId = BOOTSTRAP_BUSINESS_ID ?? (await resolveBusinessId(page));
    ...
    const hasDashboardHeading = await page
      .getByRole('heading', { name: /dashboard gastronomia|painel gastrono|painel operacional/i })
      .isVisible()
      .catch(() => false);
    ...
    await page.goto(`/central/empresas/${businessId}/gastronomia/pedidos/fake-order-id`, {
      waitUntil: 'domcontentloaded',
      timeout: 60_000,
    });

    await expect(
      page.getByText(/pedido nao encontrado|nao foi possivel carregar o pedido/i),
  ...
  test('motoboy acessa central de entregas sem quebrar fluxo (entregas ou cadastro)', async ({
    ...
    await expect(motoboyOnboardingHeading.or(motoboyOnboardingButton).first()).toBeVisible({
      timeout: 20_000,
    });

    const hasOnboardingCta = await motoboyOnboardingButton.isVisible().catch(() => false);
    if (hasOnboardingCta) {
      ...
      await expect(
        page
          ...
          .or(page.getByRole('button', { name: /cadastrar como motoboy|enviar cadastro/i }))
          .first(),
      ).toBeVisible({ timeout: 20_000 });
    }
  });
