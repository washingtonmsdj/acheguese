...
test.describe("public landing routes", () => {
  ...
  test("complexo short route resolves to the canonical Complexo landing", async ({ page }) => {
    await openPublicRoute(page, "/complexo");

    await expect
      .poll(() => page.url(), { timeout: 30_000 })
      .toContain("/ba/salvador/area/complexo-do-nordeste-de-amaralina");
    await expect
      .poll(() => readBodyText(page), { timeout: 60_000 })
    ...
    await expect
      .poll(() => readTitle(page), { timeout: 30_000 })
      .toBe("Complexo do Nordeste de Amaralina | Achegue-se");
  });
  ...
  test("territorial business and services module routes use Salvador in SEO titles", async ({ page }) => {
    await openPublicRoute(page, "/empresas/ba/salvador/area/complexo-do-nordeste-de-amaralina");
    ...
    await openPublicRoute(page, "/servicos/ba/salvador/area/complexo-do-nordeste-de-amaralina");
  ...
  test("canonical territorial area routes render city, district, modules and community cockpit", async ({ page }) => {
    const routes = [
      ...
      { path: "/ba/salvador/area/complexo-do-nordeste-de-amaralina", text: /Complexo do Nordeste de Amaralina|Empresas Locais/i },
      ...
      { path: "/empresas/ba/salvador/area/complexo-do-nordeste-de-amaralina", text: /Empresas|Complexo do Nordeste de Amaralina/i },
      { path: "/comunidade/ba/salvador/area/complexo-do-nordeste-de-amaralina/feed", text: /Feed|Comunidade|Complexo do Nordeste/i },
      { path: "/comunidade/ba/salvador/area/complexo-do-nordeste-de-amaralina/grupos", text: /Grupos|Comunidade|Complexo do Nordeste/i },
    ];
    ...
    for (const route of routes) {
      ...
      await expect
        ...
        .toMatch(route.text);
      await expect
        .poll(async () => (await readBodyText(page)).includes("Local nao encontrado"), {
          timeout: 15_000,
        })
        .toBe(false);
    }
