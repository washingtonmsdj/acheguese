...
import {
  ...
  type MigrationFile,
} from "../../tools/migrations/validate-supabase-migrations";
...
describe("Security Authority service_role boundary", () => {
  ...
  it("allows only the canonical tooling Supabase helper to read service_role env values", async () => {
    const serviceRoleBoundaryPolicy = readJson<ServiceRoleBoundaryPolicy>(
      serviceRoleBoundaryPolicyPath,
    );
    const { validateServiceRoleBoundaryFiles } =
  ...
  it("rejects direct service_role env access in the typed legacy script facade", async () => {
    const serviceRoleBoundaryPolicy = readJson<ServiceRoleBoundaryPolicy>(
      serviceRoleBoundaryPolicyPath,
    );
    const { validateServiceRoleBoundaryFiles } =
  ...
  it("allows only the canonical tooling Supabase helper to create tooling clients", async () => {
    const serviceRoleBoundaryPolicy = readJson<ServiceRoleBoundaryPolicy>(
      serviceRoleBoundaryPolicyPath,
    );
    const { validateServiceRoleBoundaryFiles } =
...
describe("Security Authority Edge Function broker boundary", () => {
  ...
  it("scans runtime source files while skipping tests and the canonical helper", async () => {
    ...
    expect(
      shouldScanEdgeFunctionBrokerBoundaryFile("tools/security/check.mjs"),
    ).toBe(false);
  });
});
