...
describe('Remote E2E mutation safety integration', () => {
  ...
  it('keeps Production Business lifecycle scoped to the dedicated owner fixture', () => {
    expect(packageJson).toContain(
      '"test:e2e:business-lifecycle-authenticated": "node tools/release/run-business-lifecycle-authenticated.mjs"',
    );
    expect(packageJson).toContain(
    ...
    const lifecycleRunner = read(
      'tools/release/run-business-lifecycle-authenticated.mjs',
    );
    const playwrightConfig = read('playwright.config.ts');
    expect(lifecycleRunner).toContain(
      'E2E_BUSINESS_LIFECYCLE_AUTHENTICATED: "true"',
    );
    expect(lifecycleRunner).toContain(
      '"tests/e2e/business-lifecycle-authenticated.spec.ts"',
    );
    expect(lifecycleRunner).toContain('"--retries=0"');
    expect(lifecycleRunner).not.toContain('SUPABASE_SERVICE_ROLE_KEY');
  ...
  it('blocks known mutating Playwright suites before hooks execute on unsafe targets', () => {
    expect(playwrightConfig).toMatch(
      /getRemoteMutationTargetSafety\(\s*process\.env\.VITE_SUPABASE_URL,\s*\)/,
    );
  ...
  it('keeps mobility gate actors on the reviewed private technical registry', () => {
    for (const retiredProfileId of RETIRED_NON_TECHNICAL_PROFILE_IDS) {
      ...
      const referenceCount =
        gate5Availability.split(retiredProfileId).length - 1;
      expect(referenceCount).toBeLessThanOrEqual(1);
      if (referenceCount === 1) {
        expect(gate5Availability).toContain(
          `const ORIGINAL_TEST_RIDE_PASSENGER_ID = '${retiredProfileId}'`,
        );
        expect(gate5Availability).toContain(
