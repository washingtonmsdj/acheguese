...
describe("sensitive report server-owned commands", () => {
  ...
  it("derives active actors and protects immutable report identity", () => {
    expect(
      migration.match(/private\.current_active_profile_id\(\)/g)?.length,
    ).toBeGreaterThan(8);
  ...
  it("stores an append-only audit envelope without report prose or evidence URLs", () => {
    ...
    expect(auditFunction).not.toMatch(
      /v_new->>'(?:description|evidence_urls|admin_notes|moderator_notes)'/,
    );
  ...
  it("removes privileged actor fields from browser create contracts", () => {
    ...
    expect(rideCreateContract).not.toMatch(
      /reporterProfileId|reporterType|profile_id/,
    );
    expect(reviewService).not.toContain(
      "reporter_profile_id: input.reporter_profile_id",
    );
    expect(
      read("src/modules/classifieds/pages/ClassificadoDetailPage.tsx"),
    ).not.toContain("createReport(activeProfile.id");
    expect(
      read("src/modules/classifieds/jobs/hooks/useVagaDetail.ts"),
    ).not.toContain("createReport(activeProfile.id");
  });
