...
describe("refresh first paint", () => {
  it("applies the persisted theme before the React module starts", () => {
    ...
    expect(html).toContain("root.dataset.themeStorageKey");
    expect(html).toContain("window.localStorage.getItem(storageKey)");
    expect(html).toContain('root.classList.toggle("dark", isDark)');
    expect(html).toContain('root.classList.toggle("light", !isDark)');
    expect(html.indexOf("root.dataset.themeStorageKey")).toBeLessThan(
