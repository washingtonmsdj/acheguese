...
export interface OperationalMutationTargetSafety {
  safe: boolean;
...
export function getOperationalMutationTargetSafety(
  supabaseUrl: string | undefined,
): OperationalMutationTargetSafety {
  if (!supabaseUrl?.trim()) {
    return {
      safe: false,
      kind: 'unproven',
      reason: 'VITE_SUPABASE_URL is required to prove the mutation target.',
    };
  }
  ...
  if (!projectRef) {
    return {
      safe: false,
      kind: 'unproven',
      reason: 'Remote Supabase target cannot be proven from VITE_SUPABASE_URL.',
    };
  }
...
export function hasApprovedOperationalMutationTarget(
  supabaseUrl: string | undefined,
): boolean {
  return getOperationalMutationTargetSafety(supabaseUrl).safe;
}

export function assertApprovedOperationalMutationTarget(
  supabaseUrl: string | undefined,
): void {
  const safety = getOperationalMutationTargetSafety(supabaseUrl);
  if (!safety.safe) {
    throw new Error(`Operational remote mutation blocked: ${safety.reason}`);
  }
}
