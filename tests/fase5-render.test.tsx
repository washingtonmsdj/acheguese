 ...
 * Testes Unitários de Lógica de Componentes
 * 
 * Objetivo: Validar lógica de usePostCard e PostAdapter
 */
...
describe('FASE 5 - Lógica de Componentes', () => {
  describe('usePostCard - Lógica de formattedLocation', () => {
    it('deve priorizar location.name do JOIN', () => {
      const location = {
        name: 'Barra',
        type: 'district',
      };

      // Simular lógica do hook
      let formattedLocation = "Localização não informada";
      
      if (location && typeof location === 'object' && 'name' in location) {
        formattedLocation = location.name;
      }

      expect(formattedLocation).toBe('Barra');
    });

    it('deve usar location como string quando não for objeto', () => {
      ...
      expect(formattedLocation).toBe('Salvador');
    });

    it('deve usar fallback temporário quando location.name não existe', () => {
      const street = undefined;
      const neighborhood = 'Barra';
      const city = 'Salvador';
      ...
      const formattedLocation = parts.length > 0 ? parts.join(", ") : "Localização não informada";

      expect(formattedLocation).toBe('Barra, Salvador');
    });
  });
  ...
  describe('PostAdapter.calculateProximity()', () => {
    it('posts do mesmo location_id devem ranquear acima', () => {
      ...
      // Posts com location_id === 'location-1' devem vir primeiro
    ...
    it('posts sem location_id devem ter menor prioridade', () => {
      ...
      // Post com location_id deve vir primeiro
      expect(sorted[0].id).toBe('post-2');
      expect(sorted[1].id).toBe('post-1');
    });

    it('PostAdapter NÃO deve usar comparação textual como regra principal', () => {
      // Verificar que calculateProximity prioriza location_id
      const posts: UnifiedPost[] = [
        {
          id: 'post-1',
          type: 'text',
      ...
      // Post com location_id correto deve vir primeiro
      expect(sorted[0].id).toBe('post-1');
    });
  });
  ...
  describe('Zero Regressão', () => {
    it('PostAdapter.sortPosts() deve continuar funcionando com critério "recent"', () => {
      ...
      // Post mais recente deve vir primeiro
      expect(sorted[0].id).toBe('post-2');
      expect(sorted[1].id).toBe('post-1');
    });
    ...
    it('PostAdapter.sortPosts() deve continuar funcionando com critério "popular"', () => {
      ...
      // Post com mais likes deve vir primeiro
      expect(sorted[0].id).toBe('post-2');
      expect(sorted[1].id).toBe('post-1');
    });
  });
