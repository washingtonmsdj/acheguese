...
const exists = (filePath: string) => existsSync(resolve(repoRoot, filePath));
...
describe("Business module architecture hardening", () => {
  it("keeps public snapshot RPC and contracts core-owned", () => {
    ...
    expect(exists("src/modules/business/public/services/PublicSnapshotRpcService.ts")).toBe(false);
    expect(exists("src/modules/business/public/types/publicSnapshots.ts")).toBe(false);
  });
  ...
  it("locks the entire Business module to zero direct runtime integration access", () => {
    ...
    expect(validator).toContain("RETIRED_PUBLIC_SNAPSHOT_BRIDGES");
