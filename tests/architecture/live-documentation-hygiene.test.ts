...
describe("live documentation hygiene", () => {
  ...
  it("keeps the Gastronomy historical audit out of operational module docs", () => {
    expect(
      existsSync("src/modules/business/gastronomy/PRODUCTION_AUDIT.md"),
    ).toBe(false);
    expect(
      existsSync(
        "docs/10-archive/audits/GASTRONOMY_PRODUCTION_AUDIT_2026-09-04.md",
      ),
    ).toBe(true);
    ...
    expect(readme).not.toContain(
      "permanece em `PRODUCTION_AUDIT.md`",
    );
  });
