...
function isProductionApplicationTarget() {
  if (readEnv('VERCEL_ENV').toLowerCase() === 'production') {
    return true;
  }

  for (const name of ['PLAYWRIGHT_BASE_URL', 'VITE_PUBLIC_APP_URL']) {
...
export function linkedProductionProjectRef() {
...
export function getRemoteMutationTargetSafety(supabaseUrl) {
  if (!supabaseUrl?.trim()) {
    return {
      safe: false,
      kind: 'unproven',
...
export function hasApprovedRemoteMutationTarget(supabaseUrl) {
  return getRemoteMutationTargetSafety(supabaseUrl).safe;
}

export function assertApprovedRemoteMutationTarget(supabaseUrl) {
