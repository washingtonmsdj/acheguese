...
export function createServiceRoleClient(config?: SupabaseScriptConfig): SupabaseClient;

export function createAnonClient(config?: SupabaseScriptConfig): SupabaseClient;

export function createSupabaseScriptClient(
  config: Pick<SupabaseScriptConfig, 'envFiles' | 'key' | 'url'> & { key: string; url: string },
): SupabaseClient;
