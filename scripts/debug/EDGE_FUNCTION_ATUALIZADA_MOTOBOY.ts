// ============================================
// EDGE FUNCTION ATUALIZADA - AUTO-DISPATCH
// ============================================
// 
// COPIAR E COLAR NO SUPABASE DASHBOARD
// Edge Functions → auto-dispatch-ride → Edit
// 
// MUDANÇAS:
// 1. Busca ride_mode da corrida
// 2. Filtra motoristas por can_do_delivery quando ride_mode = 'motoboy'
// ============================================
...
serve(async (req) => {
  ...
  try {
    ...
    // Buscar dados da corrida (INCLUINDO ride_mode)
    ...
    // Buscar motoristas elegíveis (PASSANDO ride_mode)
    ...
    console.log(`[AutoDispatch] Found ${eligibleDrivers.length} eligible drivers for ride_mode: ${ride.ride_mode || 'ride'}`);
...
async function findEligibleDrivers(
  ...
  if (rideMode === 'motoboy') {
    ...
    console.log('[AutoDispatch] Filtering for motoboy (can_do_delivery = true)');
  }
  ...
  const { data: activeRides } = await supabase
    ...
    .in('status', ['driver_accepted', 'driver_arriving', 'passenger_boarded', 'in_progress', 'pickup_confirmed', 'in_delivery']);
