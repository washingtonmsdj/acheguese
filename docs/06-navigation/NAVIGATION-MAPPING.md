...
| Responsabilidade  | Home de qualquer território específico (bairro, distrito, comunidade, região). Blocos: Hoje, Resumo, Ações rápidas, Destaques, Explorar. |
| Rotas             | `/:uf/:city`, `/:uf/:city/:district`, `/comunidade/:uf/:city`, `/comunidade/:uf/:city/:district`, `/comunidade/g/:group` |
...
| Responsabilidade  | Timeline completa do território: posts, discussões, perguntas, alertas, eventos.             |
| Rotas             | `/comunidade/:uf/:city/feed`, `/comunidade/:uf/:city/:district/feed`                         |
...
| Situação          | **Futuro** — alias criado; rotas ainda importam `ComunidadePage` do core.                    |
| Remoção prevista  | Nenhuma. `ComunidadePage` permanece como implementação; apenas o *nome público* migra para `TerritoryFeedPage`. |
