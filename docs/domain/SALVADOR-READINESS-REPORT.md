...
| Navegação | **Parcialmente pronto** | O shell público ativo deriva rotas do lifecycle/registries canônicos e a árvore desconectada `CommunityTerritoryRoutes` foi aposentada. A lacuna remanescente deste relatório está nos dados/rollout territorial e em CTAs históricos da Home, não em uma segunda árvore de módulos. |
