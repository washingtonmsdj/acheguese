...
- Comunidade removeu TODOs funcionais criticos: `PostDetailModal` envia comentario real via `commentService`; o antigo `useUnifiedDetailModal`/`CivicReportService` ficou sem caller runtime e foi retirado; `DisputeMentionModal` gera `trust_events` reais para moderacao; `useDirectMessages.reportConversation` registra incidente em `TrustEventService` e bloqueia conversa de forma auditavel.
