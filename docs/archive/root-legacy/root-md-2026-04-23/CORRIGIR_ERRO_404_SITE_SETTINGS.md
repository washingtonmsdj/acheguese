# ❌ Erro 404: get_all_site_settings

## 🔴 Problema

```
POST https://xhdowzacfujckjelqhtd.supabase.co/rest/v1/rpc/get_all_site_settings 404 (Not Found)
```

**Causa:** A função RPC `get_all_site_settings` não existe no banco de dados porque a migration não foi aplicada.
...
2. Selecione seu projeto: **xhdowzacfujckjelqhtd**
...
Você deve ver 7 linhas com as configurações padrão.

### Passo 4: Recarregar a Página

1. Volte para o navegador
2. Recarregue a página `/admin/branding`
...
Armazena todas as configurações do site (logo, favicon, cores, etc.)

### Funções RPC
- `get_all_site_settings()` - Busca todas as configurações
- `get_site_setting(key)` - Busca uma configuração específica
- `upsert_site_setting(key, value, description)` - Atualiza/cria configuração

### Políticas de Segurança
- ✅ Qualquer um pode **ler** as configurações
...
### Erro: "relation site_settings already exists"
✅ Isso é normal! Significa que a tabela já foi criada. Continue com as funções.

### Erro: "permission denied for schema public"
❌ Você não tem permissão. Use uma conta com acesso de admin ao banco.

### Erro persiste após aplicar
1. Limpe o cache do navegador (Ctrl+Shift+R)
2. Verifique se a função foi criada:
   ```sql
   SELECT proname FROM pg_proc WHERE proname = 'get_all_site_settings';
   ```
3. Se não aparecer nada, execute a migration novamente

## 📝 Arquivo Original

A migration completa está em:
```
supabase/migrations/20260422000000_create_site_settings.sql
```
...
- [ ] Verifiquei que funcionou (SELECT)
- [ ] Recarreguei a página /admin/branding
- [ ] O erro 404 desapareceu
...
- ✅ Erro 404 desaparece
- ✅ Página `/admin/branding` carrega
- ✅ Você pode fazer upload de logo
- ✅ Você pode fazer upload de favicon
- ✅ Você pode mudar as cores
