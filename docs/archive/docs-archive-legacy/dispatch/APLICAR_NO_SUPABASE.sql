...
DECLARE
  ...
  v_driver RECORD;
BEGIN
  -- So dispara se mudou para searching_driver
...
BEGIN
  ...
  LOOP
    ...
    -- Verificar se atingiu maximo de tentativas
    ...
    -- Buscar proximo motorista (excluir os ja tentados)
...
-- IMPORTANTE: Execute este comando separadamente se pg_cron estiver disponivel:
-- SELECT cron.schedule('process-dispatch-timeouts', '*/10 * * * * *', 'SELECT process_dispatch_timeouts()');

-- Se pg_cron nao estiver disponivel, execute manualmente quando necessario:
