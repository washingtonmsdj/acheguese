...
-- Description: SSOT territorial - hierarquia geogrÃ¡fica canÃ´nica
...
CREATE TABLE locations (
  ...
  -- Coordenadas canÃ´nicas (para cÃ¡lculo de radius)
...
-- Ãndice Ãºnico para slug scoped por parent (corrigido para NULL e NOT NULL)
...
-- ComentÃ¡rios
COMMENT ON TABLE locations IS 'SSOT territorial - hierarquia geogrÃ¡fica canÃ´nica';
...
COMMENT ON COLUMN locations.slug IS 'Slug Ãºnico dentro do parent (kebab-case)';
COMMENT ON COLUMN locations.geographic_path IS 'Path canÃ´nico: /br/ba/salvador/pituba';
COMMENT ON COLUMN locations.canonical_lat IS 'Latitude do ponto canÃ´nico (centro geomÃ©trico)';
COMMENT ON COLUMN locations.canonical_lng IS 'Longitude do ponto canÃ´nico (centro geomÃ©trico)';
...
-- Description: ValidaÃ§Ã£o de hierarquia, prevenÃ§Ã£o de ciclos e atualizaÃ§Ã£o automÃ¡tica de path
-- Author: Geographic Foundation
-- Date: 2026-03-24
...
BEGIN
  IF NEW.parent_id IS NOT NULL AND NEW.id IS NOT NULL THEN
    -- Verificar se parent_id nÃ£o Ã© descendente de NEW.id
    IF EXISTS (
...
-- 3. Atualizar geographic_path automaticamente (ÃšNICO PONTO DE VERDADE)
...
COMMENT ON FUNCTION prevent_location_cycles() IS 'Previne referÃªncias circulares na hierarquia';
...
-- Description: Cobertura geogrÃ¡fica de entidades (businesses, services, etc)
-- Author: Geographic Foundation
-- Date: 2026-03-24
...
CREATE TABLE service_areas (
  ...
  -- Cobertura Ãºnica por entidade + localizaÃ§Ã£o + tipo
...
-- Ãndice composto para query comum: buscar entidades em localizaÃ§Ã£o
CREATE INDEX idx_service_areas_location_entity_type_status 
  ON service_areas(location_id, entity_type, status);

-- Ãndice Ãºnico parcial para garantir apenas uma cobertura primÃ¡ria por entidade
...
-- ComentÃ¡rios
COMMENT ON TABLE service_areas IS 'Cobertura geogrÃ¡fica de entidades (businesses, services, etc)';
...
COMMENT ON COLUMN service_areas.radius_km IS 'Raio em km (obrigatÃ³rio se coverage_type=radius, NULL caso contrÃ¡rio)';
COMMENT ON COLUMN service_areas.is_primary IS 'Cobertura primÃ¡ria da entidade (apenas uma por entidade)';
COMMENT ON INDEX idx_service_areas_single_primary IS 'Garante apenas uma cobertura primÃ¡ria por entidade';
-- Migration: Create module_rollouts table
-- Description: AtivaÃ§Ã£o de mÃ³dulos por localizaÃ§Ã£o com heranÃ§a
-- Author: Geographic Foundation
-- Date: 2026-03-24
...
-- Ãndice para query de heranÃ§a (buscar rollout nos ancestors)
...
-- ComentÃ¡rios
COMMENT ON TABLE module_rollouts IS 'AtivaÃ§Ã£o de mÃ³dulos por localizaÃ§Ã£o com heranÃ§a';
COMMENT ON COLUMN module_rollouts.module_key IS 'MÃ³dulo: community, business, services, mobility, classifieds, ads';
COMMENT ON COLUMN module_rollouts.location_id IS 'LocalizaÃ§Ã£o onde mÃ³dulo estÃ¡ explicitamente configurado';
COMMENT ON COLUMN module_rollouts.status IS 'Status: active, inactive (default: inactive)';
COMMENT ON COLUMN module_rollouts.config IS 'ConfiguraÃ§Ã£o opcional do mÃ³dulo (formato livre)';
COMMENT ON COLUMN module_rollouts.created_by IS 'UsuÃ¡rio que criou o rollout (admin)';
COMMENT ON COLUMN module_rollouts.updated_by IS 'UsuÃ¡rio que atualizou o rollout (admin)';
...
-- Description: Agrupamentos territoriais â€” entidade separada da hierarquia locations
-- Regra: um grupo agrega bairros (districts) de uma mesma cidade Ã¢ncora.
--        NÃƒO cria hierarquia falsa entre bairros.
-- Date: 2026-03-24
...
CREATE TABLE territorial_groups (
  ...
  -- Cidade Ã¢ncora: facilitador operacional (todos os membros pertencem a ela)
  -- NÃƒO Ã© a verdade hierÃ¡rquica â€” os membros sÃ£o.
...
COMMENT ON COLUMN territorial_groups.anchor_city_id IS
  'Cidade Ã¢ncora â€” facilitador operacional. Todos os membros devem pertencer a ela.';
...
COMMENT ON TABLE territorial_group_members IS
  'Membros de um grupo territorial. Cada membro deve ser um district da cidade Ã¢ncora.';

-- ============================================
-- CONSTRAINT: membros devem ser districts da cidade Ã¢ncora
-- ============================================
...
-- PrÃ©-requisito: seed_initial_locations jÃ¡ criou os bairros como districts de Salvador
-- Date: 2026-03-24
...
BEGIN
  ...
  IF v_salvador_id IS NULL THEN
    RAISE EXCEPTION 'Salvador nÃ£o encontrado â€” execute seed_initial_locations primeiro';
  END IF;
  ...
  IF v_nordeste_id IS NULL OR v_santa_cruz_id IS NULL
    ...
    RAISE EXCEPTION 'Um ou mais bairros do Complexo nÃ£o foram encontrados';
  END IF;

  -- Cria o grupo com ID especÃ­fico que o app espera
...
-- update_updated_at_column() jÃ¡ criada na migration 20260324000001
-- ============================================================================
...
CREATE TABLE profiles (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id          UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  -- profile_type Ã© o campo canÃ´nico (ProfileService usa profile_type)
  ...
  bio              TEXT,
  avatar_url       TEXT,
  -- localizaÃ§Ã£o legada (mantida para compatibilidade)
  neighborhood     TEXT,
  city             TEXT,
  -- localizaÃ§Ã£o canÃ´nica (SSOT territorial)
  location_id      UUID,  -- FK adicionada apÃ³s locations
  -- contato
  phone            TEXT,
  whatsapp         TEXT,
  -- status
  ...
  -- mobilidade (temporÃ¡rio â€” ver ProfileService.getActiveRideId)
  active_ride_id   UUID,
  -- gamificaÃ§Ã£o
...
-- ============================================================================
-- posts  (PostService â€” colunas exatas do service)
-- ============================================================================
CREATE TABLE posts (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  -- autor_id â†’ auth.users (legado, mantido por compatibilidade)
  autor_id          UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  -- author_profile_id â†’ profiles (canÃ´nico)
  author_profile_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  -- conteÃºdo
  texto             TEXT,   -- campo legado
  content           TEXT,   -- campo canÃ´nico
  type              TEXT NOT NULL DEFAULT 'text'
    CHECK (type IN ('text','image','video','link','poll','alerta')),
  image_url         TEXT,
  video_url         TEXT,
  images            JSONB DEFAULT '[]',
  -- localizaÃ§Ã£o legada
  city              TEXT,
  neighborhood      TEXT,
  street            TEXT,
...
-- comments  (CommentService â€” colunas exatas)
...
-- ============================================================================
-- business_data  (BusinessService â€” colunas exatas do mapper)
-- ============================================================================
...
-- ============================================================================
-- professional_data  (ProfessionalService â€” colunas exatas do mapper)
-- ============================================================================
...
-- ============================================================================
-- classifieds  (ClassifiedService â€” colunas exatas do insert)
-- ============================================================================
...
-- ============================================================================
-- events  (EventsService â€” colunas exatas)
-- ============================================================================
CREATE TABLE events (
  id                   UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- community_alerts  (CommunityAlertService â€” via RPC create_community_alert)
...
-- VIEW pÃºblica de alertas (CommunityAlertService usa community_alerts_public)
...
-- get_driver_weekly_earnings  (MobilityService â€” parÃ¢metros: p_driver_id, p_weeks)
...
--   - Registros com status 'active' NÃƒO podem ter location_id NULL
--   - Rascunhos (status != 'active') podem ter location_id NULL
--   - Constraint aplicada como CHECK com exceÃ§Ã£o para status nÃ£o-ativo
-- Date: 2026-03-24

-- â”€â”€ business_data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
-- PolÃ­tica: registro ativo sem location_id Ã© invÃ¡lido.
...
-- â”€â”€ professional_data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
-- PolÃ­tica: profissional aceitando clientes sem location_id Ã© invÃ¡lido.
...
-- â”€â”€ NOTA IMPORTANTE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
-- Esta migration FALHARÃ se houver registros ativos sem location_id apÃ³s o backfill.
-- Se isso ocorrer:
...
--   3. Ou mover registros problemÃ¡ticos para status != 'active' temporariamente
--   4. EntÃ£o re-executar esta migration
--
-- VerificaÃ§Ã£o prÃ©via recomendada:
...
COMMENT ON COLUMN territorial_highlights.territory_type IS 'Tipo de territÃ³rio: group (grupo territorial) ou location (bairro)';
COMMENT ON COLUMN territorial_highlights.territory_ref_id IS 'ID do territÃ³rio (group_id ou location_id)';
COMMENT ON COLUMN territorial_highlights.position IS 'PosiÃ§Ã£o de ordenaÃ§Ã£o (menor = primeiro)';
...
COMMENT ON COLUMN classifieds.titulo IS 'TÃ­tulo do classificado';
-- Migration: Create territory_ai_content table
-- Description: Tabela para armazenar conteÃºdo gerado por IA para territÃ³rios
-- Date: 2026-03-25
...
-- ComentÃ¡rios
COMMENT ON TABLE territory_ai_content IS 'ConteÃºdo gerado por IA para territÃ³rios (bairros/grupos)';
COMMENT ON COLUMN territory_ai_content.territory_slug IS 'Slug do territÃ³rio (ex: complexo-do-nordeste-de-amaralina)';
COMMENT ON COLUMN territory_ai_content.territory_name IS 'Nome completo do territÃ³rio';
COMMENT ON COLUMN territory_ai_content.ai_generated_at IS 'Data/hora da geraÃ§Ã£o do conteÃºdo por IA';
COMMENT ON COLUMN territory_ai_content.is_manual_override IS 'Se o conteÃºdo foi editado manualmente (sobrescreve IA)';
...
BEGIN
  ...
  IF v_complexo_id IS NOT NULL THEN
    ...
    (
      ...
      'Oficinas de capacitaÃ§Ã£o para jovens da comunidade',
      'users',
      '#3b82f6',
      2,
      'active'
    ...
    (
      'group',
      v_complexo_id,
      'Campanha de VacinaÃ§Ã£o',
      'Posto de saÃºde com vacinaÃ§Ã£o gratuita atÃ© dia 30',
      'heart',
      '#ef4444',
      3,
      'active'
    ...
    (
      ...
      'EspaÃ§o de leitura e estudo com acervo renovado',
      'book-open',
      '#8b5cf6',
      4,
      'active'
  ...
  ELSE
    RAISE NOTICE 'Grupo territorial nÃ£o encontrado';
  END IF;
END $$;
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Grupo com ID % nÃ£o encontrado', v_old_id;
  END IF;
END $$;
-- ============================================================================
...
-- Adicionar role de admin para o usuÃ¡rio washingtonmsdj@gmail.com
-- Esta migration Ã© idempotente (pode ser executada mÃºltiplas vezes sem erro)
...
BEGIN
  ...
  -- Se o usuÃ¡rio existe, adicionar role de admin
