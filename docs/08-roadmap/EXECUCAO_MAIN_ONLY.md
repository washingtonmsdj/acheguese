...
- [~] Reconciliação de migrations avançou no PR #225 sem executar DDL: duas identidades comprovadamente equivalentes foram reconstruídas do ledger remoto. Estado auditado após a reconciliação de Safety G71/G72/G75–G80: 686 locais, 666 remotas, 666 exatas, 20 local-only e 0 remote-only.
...
- [~] O ledger de migrations avançou novamente sem executar DDL: G71/G72/G75–G80 foram alinhadas às oito identidades `reconcile_*` realmente registradas no Supabase após prova de equivalência token-a-token. Estado: 686 locais / 666 remotas / 666 exatas / 20 local-only / 0 remote-only. Ainda não executar `db push`.
