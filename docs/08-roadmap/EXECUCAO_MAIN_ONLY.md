...
- #350 integrou uma pausa de Notificações, posteriormente classificada como decisão arquitetural incorreta: Notificações é capability horizontal do produto e deve permanecer ativa; o corte correto ocorre nos produtores/providers pertencentes a domínios pausados;
...
Notificações é horizontal e permanece ativa independentemente das verticais. Mensagens também é horizontal: depende de Auth + Perfis, enquanto o registry de providers registra **somente Business Direct Messaging** no MVP. Classificados e Community preservam seus agregados, mas não entram na Inbox enquanto seus domínios estiverem pausados.
...
- Inbox de Notificações usa `/notificacoes` e preferências usam `/conta/notificacoes`; `/settings/email-logs` permanece fora do shell público do MVP;
