...
O plano continua ativo: o conector Supabase confirmou o projeto ativo e RLS nas tabelas de intake, mas o CLI local ainda não tem `SUPABASE_ACCESS_TOKEN` para verificar Edge secrets. `anon`/`authenticated` não têm `INSERT` efetivo nas duas tabelas, mas policies legadas continuam; `authenticated` ainda pode alterar `professional_stats`, o trigger G39 não existe e as migrations de cutover seguem bloqueadas pelos smokes LIVE e pelas evidências operacionais. A produção Vercel está `READY` no SHA `78c60e7` de `main`; o código atual de `main` mudou apenas em docs/testes desde esse deploy. O deployment de preview do SHA funcional `3f161228` também está `READY`, mas os deployments documentais `ab66ab4` e `313aec7` foram cancelados pelo `Ignored Build Step`; o check GitHub `SUCCESS` de `313aec7` não é prova de build do HEAD. As Edge Functions ativas divergem do leitor de body do branch, e a lista remota de migrations precisa ser reconciliada com os arquivos locais antes de aplicar DDL. A autoridade de release segue bloqueada por exceções expiradas e snapshot de recuperação stale; os secrets GitHub ainda estão no escopo do repositório; hosted CI está limitado por orçamento, com checks falhando antes do primeiro step (`steps=[]`), incluindo scanner de segurança. Certificação pesada segue `QUEUED`. Não houve migration, deploy de Edge Function, merge nem habilitação de Mobilidade. Detalhes no [checkpoint de 2026-09-17, atualizado em 2026-09-18](./checkpoints/2026-09-17-pr117-urgent-hardening.md).
