...
O plano continua ativo: o conector Supabase confirmou o projeto ativo e RLS nas tabelas de intake, mas o CLI local ainda não tem `SUPABASE_ACCESS_TOKEN` para verificar Edge secrets. `anon`/`authenticated` não têm `INSERT` efetivo nas duas tabelas, mas policies legadas continuam; `authenticated` ainda pode alterar `professional_stats`, o trigger G39 não existe e as migrations de cutover seguem pendentes de frontend LIVE + smoke. As Edge Functions ativas divergem do leitor de body atual do branch, e a lista remota de migrations precisa ser reconciliada com os arquivos locais antes de aplicar DDL. A autoridade de release segue bloqueada por exceções expiradas e snapshot de recuperação stale; os secrets GitHub ainda estão no escopo do repositório; hosted CI está limitado por orçamento e o check Vercel do HEAD está `PENDING`. Os checks do SHA `3f1612287a36d4e56e3f7d06607b5340e36d872e` e do checkpoint documental `ab66ab4ad8958b282cf20e1e0500721bc4c8ad73` receberam anotação de bloqueio de orçamento antes do primeiro step (`steps=[]`), incluindo scanner de segurança; portanto, não há resultado hospedado de teste. Certificação pesada segue `QUEUED`. Não houve migration, deploy, merge nem habilitação de Mobilidade. Detalhes no [checkpoint de 2026-09-17, atualizado em 2026-09-18](./checkpoints/2026-09-17-pr117-urgent-hardening.md).
