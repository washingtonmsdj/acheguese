...
O plano continua ativo: falta validar nomes de secrets Supabase com token de leitura; a autoridade de release está bloqueada por exceções expiradas e snapshot de recuperação stale; os secrets GitHub ainda estão no escopo do repositório; hosted CI está limitado por orçamento e o check Vercel do HEAD está `PENDING`. Na atualização remota de 2026-09-18, os checks do SHA `3f1612287a36d4e56e3f7d06607b5340e36d872e` receberam anotação de bloqueio de orçamento antes do primeiro step (`steps=[]`), incluindo o scanner de segurança; portanto, não há resultado hospedado de teste. Certificação pesada segue `QUEUED`. Não houve migration, deploy, merge nem habilitação de Mobilidade. Detalhes no [checkpoint de 2026-09-17, atualizado em 2026-09-18](./checkpoints/2026-09-17-pr117-urgent-hardening.md).
