...
Baseline de código deste checkpoint: `3b52d36227f887554f2f53687127d33b71a54ed0`

## Objetivo

Fechar duas fragilidades encontradas durante a consolidação G39 sem reabrir autoridade de browser:
...
## Residual E2E identificado

`tests/e2e/professional-leads-operational.spec.ts` ainda cria a **fixture técnica** de `professional_leads` com `INSERT` autenticado direto. Isso não é caller runtime do produto, mas ficará incompatível com o cutover que revoga `INSERT` de `anon/authenticated`.

Não enfraquecer o cutover para acomodar o teste. O caminho correto é migrar a criação da fixture para uma autoridade técnica isolada (`service_role` somente em target E2E aprovado) e manter um smoke separado do fluxo público passando por `create-professional-lead` + Turnstile.

## Validação disponível neste ambiente

Os pushes dispararam `SSOT Enforcement` e `Security Check`, porém os jobs encerraram como `failure` antes de executar steps e sem logs de job disponíveis. Logo, esse sinal não prova erro de código nem aprovação dos gates.
...
5. E2E/fixtures não dependerem de `INSERT` browser que o cutover revogará;
6. security/architecture/typecheck/build executados de verdade e verdes.

## Próxima ação recomendada

1. migrar a fixture direta de `professional_leads` para autoridade admin isolada, sem reabrir grants ao browser;
2. executar os testes focados G39/G40 e o broker transversal assim que houver executor disponível;
3. executar gates globais de arquitetura/segurança/typecheck/build;
4. publicar o frontend e provar os dois smokes;
5. somente então executar o cutover transacional de leads + `professional_stats`.
