...
- No SHA `0ecaf2a78ac0e3ab0d26f1f0cac70ad881f0a61b`, os cinco workflows hospedados falharam antes de alocar runner; `steps=[]`, `runner_id=0` e a anotação do check informa: “The job was not started because an Actions budget is preventing further use.” O rerun desses cinco workflows repetiu a mesma falha de início.
- `Heavy PR Certification (Auto)` permaneceu em fila. O check Vercel falhou por limite de builds e indicou nova tentativa em 24 horas; não há deploy `READY` no SHA candidato.

Os lotes estruturais B–E da issue #51 estão concluídos neste checkpoint. A ausência de credenciais continua sendo uma limitação do ambiente para executar os dois fluxos E2E autenticados. A certificação hospedada e a prova de deploy do SHA candidato seguem pendentes por bloqueios de budget/limite externos; não houve aprovação de release, merge nem deploy.
