...
O fetch subjacente ainda pode terminar no navegador, mas o contrato de estado do consumidor deixa de ser infinito. Cancelamento/AbortSignal da fonte externa pode ser endurecido em uma rodada futura.
...
- `tests/architecture/territory-polygon-resilience.test.ts`: exige timeout compartilhado/limpeza da promise e protege degradação não bloqueante da `/`.
...
2. avaliar AbortSignal real nas consultas externas de boundary para interromper também o trabalho subjacente, não apenas o estado do consumidor;
