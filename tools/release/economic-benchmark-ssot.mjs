  ...
  if (hasPsql) {
    ...
    try {
      ...
      try {
        ...
        runAllowFail("node", ["tools/release/diagnose-economic-db-connectivity.mjs"]);
        fail(
...
function compare() {
  run("node", ["tools/release/compare-economic-benchmark.mjs", BEFORE_FILE, AFTER_FILE, SUMMARY_FILE]);
}

function report() {
  run("node", ["tools/release/generate-economic-staging-report.mjs", SUMMARY_FILE, REPORT_DRAFT_FILE]);
}

function finalize() {
  run("node", ["tools/release/finalize-economic-benchmark.mjs"]);
}
