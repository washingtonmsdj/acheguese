...
- [ ] criar provenance reutilizável por seção/fato antes de preencher infraestrutura em massa; um único `school_source_url` não é suficiente para provar endereço, contato, matrícula e dezenas de instalações com ritmos de atualização diferentes.
...
- [ ] **Sugerir correção factual**: criar fluxo separado de denúncia para nome/endereço/telefone/site/horário/rede/INEP/etapas/matrícula/infraestrutura desatualizados, com campo/fato, valor proposto, fonte/evidência e provenance. Não enviar erro factual comum para a fila de abuso/moderação.
- [ ] **Mantenedora/rede/Secretaria**: criar autoridade hierárquica para uma Prefeitura/Secretaria/grupo escolar administrar um conjunto de unidades sem repetir membership escola por escola. O acesso herdado deve ser revogável no nível da rede e nunca transformar a organização em owner individual de cada unidade.
...
- [ ] manter a ordem principal: cobertura pública Salvador + provenance/read model -> correções factuais -> autoridade de mantenedora/rede -> reviews/comparador -> particulares em escala. Não atrasar a carga municipal para construir ranking ou monetização.
