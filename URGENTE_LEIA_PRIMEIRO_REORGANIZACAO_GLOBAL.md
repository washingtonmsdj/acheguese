...
- [x] hierarquia empresarial permanece separada de autorização: mutação estrutural de rede exige Proprietário estrutural e `parent_business_id` não concede acesso herdado. O Organization Scope institucional foi fechado por `20260907005404_add_business_institution_management_scope_g6.sql` + `20260907010116_add_business_institution_scope_admin_model_g6.sql`: Prefeitura/Secretaria/mantenedora continua sendo um Profile canônico, gestores ficam uma única vez em `profile_members`, o scope privado herda gestão para escolas públicas sem copiar memberships, grant/revoke são admin-reviewed + service-role-only, o `admin-business-rpc` v10 mantém `verify_jwt=true`, a Gestão de Empresas ganhou UI explícita para conceder/revogar com evidência e `types.generated.ts` foi regenerado integralmente do Supabase vivo; ownership estrutural e pessoas/acessos continuam exclusivos do owner;
